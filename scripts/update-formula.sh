...
#   scripts/update-formula.sh v0.1.0-build.42
# ──────────────────────────────────────────────────────────────

TAG="${1:?Usage: $0 <release-tag> [repo]  (e.g. v0.1.0-build.42 okx/onchainos-skills)}"
...
# Extract clean version number from tag (strip v prefix and -build.N suffix)
VERSION=$(echo "$TAG" | sed 's/^v//' | sed 's/-build\..*//')
BIN="onchainos"
