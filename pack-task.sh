...
#
# 4 个二进制都启用 debug-log feature，并使用 CLI 默认域名。
#
# 交叉编译依赖(只需装一次):
...
echo "→ [darwin-arm64] cargo install onchainos (host, --debug, default base URL, debug-log)"
( cd "$CARGO_DIR" && cargo install --path . --force --debug --features debug-log )
...
build_cross_target() {
  ...
  case "$target" in
    *-apple-darwin)
      ...
      ( cd "$CARGO_DIR" && cargo build --target "$target" --features debug-log )
      ;;
    *)
      ...
      if [ -n "$HAVE_ZIGBUILD" ]; then
        ...
        ( cd "$CARGO_DIR" && cross build --target "$target" --features debug-log ) || true
      fi
      ;;
  esac
...
会失败。请在你的 Terminal.app 里**预编译一次**(只跑一次,以后 cache 在了 pack 走 reuse):
  ...
  cd $CARGO_DIR
  cargo zigbuild --target $target --features debug-log
...
cat > "$STAGE/install-agent-task.sh" <<'INSTALL_AGENT_TASK'
#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
INSTALL_DIR="${ONCHAINOS_INSTALL_DIR:-$HOME/.local/bin}"
SKILLS_DIR="${ONCHAINOS_SKILLS_DIR:-$HOME/.onchainos/skills}"
AGENTS_SKILLS_DIR="${ONCHAINOS_AGENTS_SKILLS_DIR:-$HOME/.agents/skills}"

case "$(uname -s):$(uname -m)" in
  Darwin:arm64)  BINARY_NAME="onchainos-darwin-arm64" ;;
  Darwin:x86_64) BINARY_NAME="onchainos-darwin-x64" ;;
  Linux:x86_64)  BINARY_NAME="onchainos-linux-x64" ;;
  Linux:aarch64) BINARY_NAME="onchainos-linux-arm64" ;;
  *)
...
[ -x "$SCRIPT_DIR/$BINARY_NAME" ] || {
  echo "✗ package does not contain executable $BINARY_NAME" >&2
  exit 1
}
[ -d "$SCRIPT_DIR/skills" ] || {
  echo "✗ package does not contain skills/" >&2
  exit 1
}

mkdir -p "$INSTALL_DIR" "$(dirname "$SKILLS_DIR")" "$AGENTS_SKILLS_DIR"
install -m 0755 "$SCRIPT_DIR/$BINARY_NAME" "$INSTALL_DIR/onchainos"

rm -rf "$SKILLS_DIR"
cp -R "$SCRIPT_DIR/skills" "$SKILLS_DIR"

for skill_dir in "$SKILLS_DIR"/okx-*; do
  [ -d "$skill_dir" ] || continue
  skill_name="$(basename "$skill_dir")"
  rm -rf "$AGENTS_SKILLS_DIR/$skill_name"
  ln -s "$skill_dir" "$AGENTS_SKILLS_DIR/$skill_name" 2>/dev/null || cp -R "$skill_dir" "$AGENTS_SKILLS_DIR/$skill_name"
done

version="$("$INSTALL_DIR/onchainos" --version 2>/dev/null || true)"
skill_count="$(find "$SKILLS_DIR" -mindepth 1 -maxdepth 1 -type d | wc -l | tr -d ' ')"

echo "✓ installed onchainos to $INSTALL_DIR/onchainos"
echo "  version: ${version:-unknown}"
echo "  skills:  $skill_count copied to $SKILLS_DIR"
echo "  linked:  $AGENTS_SKILLS_DIR/okx-*"
case ":$PATH:" in
  *":$INSTALL_DIR:"*) ;;
  *)
    echo
    echo "Add $INSTALL_DIR to PATH if 'onchainos' is not found in new shells."
    ;;
esac
INSTALL_AGENT_TASK
chmod 0755 "$STAGE/install-agent-task.sh"
