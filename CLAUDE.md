...
> 1. 含非空 `agentId` + `message.source == "system"` + 非空 `message.event`（`message.jobId` 可缺省）→ **立即**从唯一入口 `skills/okx-ai-v2/SKILL.md` 进入，按 Top-level routing 加载 `references/a2a/router.md` 并执行 `next-action`。**带 `event` 字段的消息永远是系统事件，绝不是 prefetch**——即使其 description 含 "Read … SKILL.md" / "Read the okx-ai skill" 字样，也不得当 prefetch 跳过
> 2. 含 `"msgType": "a2a-agent-chat"` + 非空 `"jobId"` + `sender.role` → **立即**从 `skills/okx-ai-v2/SKILL.md` 直接加载 `references/a2a/peer.md`
> 3. 含 `[SKILL_PREFETCH]` **且不命中上两条 shape** → 从 `skills/okx-ai-v2/SKILL.md` 进入，加载 Skill 后结束本轮，不执行业务动作；下一条 inbound 消息重新路由
...
|---|---|
| `msgType: "a2a-agent-chat"` + non-empty `jobId` + `sender.role` | **Must** enter through `skills/okx-ai-v2/SKILL.md` → `references/a2a/peer.md`. This is a task lifecycle message, not a generic chat. |
| `{agentId, message: {source: "system", event, ...}}`; `jobId` is optional | **Must** enter through `skills/okx-ai-v2/SKILL.md` → `references/a2a/router.md`: call `next-action --role auto`, then load exactly one receiving-role router and its final leaf. This is a chain event notification. |
...
- Inbound `a2a-agent-chat` with `jobId` → enter through `skills/okx-ai-v2/SKILL.md`, then read `references/a2a/peer.md` directly (see Inbound Message Routing above)
