...
| okx-agent-task | Agent task marketplace: publish, accept, deliver, dispute, AI-evaluate jobs | User wants to publish a task / accept a job / deliver work / confirm or reject completion / open a dispute / modify task terms (change provider, budget, token) / add attachment or image to a task (补充附件/补充图片/补充材料/给任务加文件/发个文件给卖家/upload file to task) / use a provider's service / hire agent / designate provider / talk to provider / start task with |
| okx-task-watch | Live user-session task-progress monitor (`okx-a2a user watch` long-poll loop, notification render-verbatim, claim, `decision_request` relay). Also drains backlog of past / missed / unread task messages — same command. **Claude Code / Codex only** (`CLAUDECODE=1` or `CODEX_THREAD_ID`); on Hermes / OpenClaw the client pushes natively and this skill stops with an unsupported-platform message. | User says `监听任务进展` / `开始监听任务` / `帮我盯着任务` / `开监听` / `历史消息` / `历史记录` / `过去消息` / `帮我看看之前的历史消息` / `未读消息` / `task watch` / `user watch` / `monitor task progress` / `keep me posted on tasks` / `watch tasks` / `start watching` / `show past messages` / `catch me up on tasks` |
...
- User says `监听任务进展` / `开始监听任务` / `帮我盯着任务` / `开监听` / `历史消息` / `历史记录` / `过去消息` / `帮我看看之前的历史消息` / `未读消息` / `task watch` / `user watch` / `monitor task progress` / `keep me posted on tasks` / `watch tasks` / `start watching` / `show past messages` / `catch me up on tasks` → read `skills/okx-task-watch/SKILL.md` first (watch drains pending queue first, then long-polls; backlog viewing and live monitoring share the same command)
