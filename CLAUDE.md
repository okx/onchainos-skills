...
> 1. 含非空 `agentId` + `message.source == "system"` + 非空 `message.event`（`message.jobId` 可缺省）→ **立即**从唯一入口 `skills/okx-ai/SKILL.md` 进入，按 Top-level routing 加载 `references/a2a/router.md` 并执行 `next-action`。**带 `event` 字段的消息永远是系统事件，绝不是 prefetch**——即使其 description 含 "Read … SKILL.md" / "Read the okx-ai skill" 字样，也不得当 prefetch 跳过
> 2. 含 `"msgType": "a2a-agent-chat"` + 非空 `"jobId"` + `sender.role` → **立即**从 `skills/okx-ai/SKILL.md` 直接加载 `references/a2a/peer.md`
> 3. 含 `[SKILL_PREFETCH]` **且不命中上两条 shape** → 从 `skills/okx-ai/SKILL.md` 进入，加载 Skill 后结束本轮，不执行业务动作；下一条 inbound 消息重新路由
...
|---|---|
| `msgType: "a2a-agent-chat"` + non-empty `jobId` + `sender.role` | **Must** enter through `skills/okx-ai/SKILL.md` → `references/a2a/peer.md`. This is a task lifecycle message, not a generic chat. |
| `{agentId, message: {source: "system", event, ...}}`; `jobId` is optional | **Must** enter through `skills/okx-ai/SKILL.md` → `references/a2a/router.md`: call `next-action --role auto`, then load exactly one receiving-role router and its final leaf. This is a chain event notification. |
| Contains `"Please send a request to this endpoint."` | Route to `okx-agent-payments-protocol` skill. |

These envelope-based routes take absolute priority over the skill table below. Do NOT handle these envelopes with only XMTP tools. System envelopes use the v2 A2A entry router and its selected role leaf; Peer envelopes use the dedicated Peer leaf directly.

> Note: `okx-agent-identity` / `okx-agent-task` / `okx-task-watch` / `okx-agent-chat` no longer exist as separate skill directories. `skills/okx-ai/SKILL.md` is the unique entry for identity, task marketplace, task watch, and agent chat; its domain references live under `skills/okx-ai/references/`. CLI playbook output must enter through that SKILL and may then name the selected v2 domain reference; it must not point back to the retained legacy tree.
...
| okx-guide | Onboarding & guide hub: install/update/upgrade routing (forced `preflight --force` + unified install-success template), Onchain OS intro + welcome banner, OKX.AI intro + role-registration routing, and customer-support / Help Center guidance. Classifies the intent and routes to the right sub-flow. | User wants to install / update / upgrade / reinstall Onchain OS, asks "what is this / what can it do / how do I use it / getting started / I'm new", asks about OKX.AI (or any spelling variant), or wants customer service / talk to a human / file a complaint / give feedback / help docs / FAQ / report a bug |
...
| okx-ai | ERC-8004 on-chain Agent identity (register/update/search/rate/service-list) + agent task marketplace (publish/accept/deliver/dispute/**subscription tasks — subscribe/auto-renew/trial cancel/reject/claim refund**) + live task-progress monitor, unified. **Claude Code / Codex only** for the monitor half (`CLAUDECODE=1` or `CODEX_THREAD_ID`); on Hermes / OpenClaw the client pushes task notifications natively. | User wants to register/create/update/deactivate/activate/search agents (roles — User: User / User Agent / Buyer / Client / 用户 / 买家 / 买方; ASP: ASP / Provider / Provider Agent / Seller / Merchant / 提供者 / 商家 / 服务提供商 / 卖家 / 卖方; Evaluator / 仲裁者 — e.g. "注册ASP", "register ASP", "建ASP身份", "注册买家"), submit or view feedback, list agent services; publish a task / accept a job / deliver work / confirm or reject completion / open a dispute / modify task terms (change provider, budget, token) / add attachment or image to a task / hire agent / 指定服务商 / **subscribe task / subscription task / auto-renew / trial cancel / reject delivery / claim refund / my subscription tasks**; or says `监听任务进展` / `帮我盯着任务` / `历史消息` / `未读消息` / `未决策` / `待决策` / `task watch` / `user watch` / `monitor task progress` / `catch me up on tasks` / `outstanding decisions` / `pending decisions` |
...
Routing:
- **User session** free-form task intent (publish / designated-provider / attachment / terms / deliverables) → enter through `skills/okx-ai/SKILL.md`, then `references/a2a/router.md` → `references/a2a/user/router.md`; load only the selected final leaf
- User asks for their subscriptions (我的订阅 / 订阅列表 / 订阅详情 / my subscriptions — AI-service context, no 402/URL) → enter through `skills/okx-ai/SKILL.md`, then route through `references/a2a/router.md` → `references/a2a/user/router.md` to `references/a2a/user/subscription.md`
- Inbound `a2a-agent-chat` with `jobId` → enter through `skills/okx-ai/SKILL.md`, then read `references/a2a/peer.md` directly (see Inbound Message Routing above)
- User says `监听任务进展` / `开始监听任务` / `帮我盯着任务` / `开监听` / `历史消息` / `历史记录` / `过去消息` / `帮我看看之前的历史消息` / `未读消息` / `未决策` / `待决策` / `没有决策` / `未处理` / `待处理` / `没有处理` / `task watch` / `user watch` / `monitor task progress` / `keep me posted on tasks` / `watch tasks` / `start watching` / `show past messages` / `catch me up on tasks` / `outstanding decisions` / `pending decisions` → enter through `skills/okx-ai/SKILL.md`, then route to `references/runtime/watch.md` (watch drains pending queue first then long-polls for live monitoring; outdated-list batch-renders un-replied decisions on demand)
...
- Identity lifecycle intent—register / create / update / activate (上架) / deactivate (下架) / search an agent identity (ASP / User / Evaluator / 服务提供商 / 卖家 / 买家 / 用户 / 仲裁者 and aliases)—belongs to `okx-ai`. This holds even when the request also says "Onchain OS" (e.g. "用 Onchain OS 上架我的ASP"): identity lifecycle verbs win. Raw transaction broadcasting / simulating / tracking belongs to the `okx-agentic-wallet` gateway domain.
