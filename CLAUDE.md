...
**Quick tiebreaker `okx-activity` vs `okx-growth-competition`**: only hackathon / 交易黑客松 / 黑客松 → `okx-activity` (hackathon activity → `hackathon register`); any other competition → `okx-growth-competition` (`competition join`).
