...
- **skills/** — 13 onchainos CLI skill definitions (each is a `SKILL.md` with YAML frontmatter + CLI command reference)
  - Note: real-time WebSocket watch commands (`tracker watch`) are part of `okx-dex-signal`, not a separate skill
...
| okx-dex-signal       | Smart money / KOL / whale activity tracking, buy signal alerts, leaderboard, real-time WebSocket watch | User asks what smart money/whales/KOLs are buying or trading (tracker), wants aggregated buy signal alerts (信号), wants top traders ranked by PnL/win rate/volume (牛人榜), or wants real-time continuous trade feed (实时监控/盯盘/watch live) |
