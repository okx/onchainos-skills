...
impl CreateTaskParams {
    fn validate(&self) -> Result<ValidatedParams> {
        ...
        if self.provider.trim().is_empty() {
            bail!("A designated provider is required. Confirm a service-match result and run task-create-prepare first.");
        }
        if self.service_id.trim().is_empty() {
            bail!("A service id is required. Confirm a service-match result and run task-create-prepare first.");
        }
