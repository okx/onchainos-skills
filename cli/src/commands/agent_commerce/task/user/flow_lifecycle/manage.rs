...
- `existingSubscription != null` → require top-level `duplicateSubscription`. A missing object is a hard stop. Do **not** call `service-list`, render the normal confirmation card, or continue to Steps 3.5–6. Do not query, list, or suggest the ASP's other services.
  ...
  - If the user chooses **Restore listening**, keep `<jobId>` as the explicit current subscription and read `skills/okx-ai-v2/references/a2a/user/subscription-manage.md` §Signal-receipt watch entry directly. This is receipt restoration, not an execution-policy review, so its first authorization gate omits `--review-existing`.
...
- Route `nextAction.id=watch_task` directly to `skills/okx-ai-v2/references/runtime/watch.md` immediately; do not re-enter `SKILL.md` or the A2A router.
