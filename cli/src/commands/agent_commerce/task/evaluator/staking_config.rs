...
pub async fn handle_staking_config(client: &mut TaskApiClient, agent_id: &str) -> Result<()> {
    ...
    println!(
        "  minCumulativeStakeOkb       : {} OKB",
        cfg.min_cumulative_stake_okb
    );
    println!(
        "  partialUnstakeMinRetainOkb  : {} OKB",
    ...
    println!(
        "  slashedCooldownHours        : {}",
        cfg.slashed_cooldown_hours()
    );
    Ok(())
}
