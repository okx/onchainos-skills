...
fn dispute_next_action(job_id: &str, job_status: &str, _agent_id: &str) -> Option<String> {
    let body = match job_status {
        ...
        "reveal_started" => "[Current Status] reveal_started\n\n\
             Extract `jobId` and `agentId` from the inbound envelope top level and execute reveal (if `jobId` missing → output `missing jobId in payload; abort` log and end):\n\
             ```bash\n\
        ...
        "vote_revealed" => "[Current Status] vote_revealed\n\n\
             [Action] None; do not notify the user.\n".to_string(),
