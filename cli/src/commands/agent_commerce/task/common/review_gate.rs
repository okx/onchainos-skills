...
//! - `next-action --event job_submitted --jobStatus job_submitted --role buyer` → writes `pending`
//! - `next-action --event approve_review --jobStatus approve_review --role buyer` → `pending` → `approved`
//!
//! Check points:
        ...
        Ok(content) => {
            bail!(
                "review-gate state error: expected 'pending', got '{}'. \
                 Please run next-action --event job_submitted --jobStatus job_submitted first.",
                content.trim()
            );
        }
        Err(_) => {
            bail!(
                "review-gate file does not exist (job_submitted flow was not executed). \
                 Please call next-action --event job_submitted --jobStatus job_submitted --role buyer first."
            );
        }
    }
}
        ...
        Ok(content) if content.trim() == "pending" => {
            bail!(
                "User has not made a review decision yet (review-gate = pending). \
                 ...
                 call `next-action --jobid <jobId> --event user_decision_job_submitted --jobStatus user_decision_job_submitted --role buyer --agentId <agentId> --data \"<message.data>\"` — \
                 the returned playbook will instruct you to call `next-action --event approve_review --jobStatus approve_review` (when the user approves) or `--event reject_review --jobStatus reject_review` (when the user rejects)."
            );
        }
        Ok(content) => {
        ...
        Err(_) => {
            bail!(
                "review-gate file does not exist. In escrow mode you must run \
                 next-action --event job_submitted --jobStatus job_submitted review flow first. \
