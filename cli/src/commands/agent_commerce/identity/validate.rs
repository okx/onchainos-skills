...
mod tests {
    ...
    fn clean_provider_passes() {
        ...
        let r = run_validation("provider", Some("Summarizer Bot"), Some("A helpful agent."), Some(&service));
    ...
    fn long_bilingual_name_not_blocked_by_cjk_length_cap() {
        ...
        let r = run_validation("provider", Some("健身 \u{00B7} Fitness Coach Pro"), None, None);
    ...
    fn hex_in_service_description_emits_d7_not_duplicate_u2() {
        ...
        let desc_codes: Vec<&str> = r
            ...
            .map(|f| f.code.as_str())
            .collect();
        assert!(desc_codes.contains(&"D7"), "expected D7, got {desc_codes:?}");
        assert!(!desc_codes.contains(&"U2"), "U2 must not duplicate D7, got {desc_codes:?}");
    }
