...
impl McpServer {
    ...
    async fn token_search(
        ...
        match token::fetch_search(&mut *self.client.lock().await, &p.query, chains).await {
            Ok(data) => ok(data),
            Err(e) => err(e),
        }
    }
    ...
    async fn token_holders(
        ...
        let chain_index = p
            ...
            .map(crate::chains::resolve_chain)
            .unwrap_or_else(|| crate::chains::resolve_chain("ethereum").to_string());
        match token::fetch_holders(&mut *self.client.lock().await, &p.address, &chain_index, p.tag_filter).await {
            Ok(data) => ok(data),
            Err(e) => err(e),
        }
    }
    ...
    async fn token_top_trader(
        ...
        let chain_index = p
            ...
            .map(crate::chains::resolve_chain)
            .unwrap_or_else(|| crate::chains::resolve_chain("ethereum").to_string());
        match token::fetch_top_trader(&mut *self.client.lock().await, &p.address, &chain_index, p.tag_filter).await {
            Ok(data) => ok(data),
            Err(e) => err(e),
        }
    }
