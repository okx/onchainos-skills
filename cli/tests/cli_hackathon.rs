//! Integration tests for `onchainos hackathon register` — OKX.AI trading
//! hackathon Trading ASP registration.
...
/// under `cli/target/test_tmp/cli_hackathon/` — NOT `tempfile::tempdir()`
...
#[test]
fn hackathon_register_web3_not_logged_in_errors() {
...
#[test]
fn hackathon_register_cefi_missing_uid_errors() {
    let output = onchainos()
        .args([
            "hackathon",
            "register",
...
#[test]
fn hackathon_register_web3_invalid_address_errors() {
    let output = onchainos()
        .args([
            "hackathon",
            "register",
...
#[test]
fn hackathon_register_missing_agent_id_is_usage_error() {
    let output = onchainos()
        .args(["hackathon", "register", "--account-type", "web3"])
        .output()
        .expect("failed to execute");
...
#[test]
fn hackathon_register_invalid_account_type_is_usage_error() {
    let output = onchainos()
        .args([
            "hackathon",
            "register",
...
#[test]
fn hackathon_register_cefi_golden_happy_path() {
...
#[test]
fn hackathon_register_non_qualifying_agent_is_rejected() {
...
#[test]
fn hackathon_register_network_error_surfaces_ok_false() {
    let Some(agent_id) = std::env::var("HACKATHON_QUALIFYING_AGENT_ID")
        .ok()
        .filter(|s| !s.is_empty())
    else {
