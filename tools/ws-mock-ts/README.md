...
ws-mock server（ws://127.0.0.1:9000）  ← 模拟 XMTP
    ↕
mock-buyer / mock-seller / mock-evaluator  ← 交互式测试工具
...
#   target/release/mock-evaluator  — 仲裁者测试工具
```
...
```bash
./target/release/mock-evaluator [--buyer-addr <买家钱包地址>]
```
...
mock-seller 和 mock-evaluator 需要知道买家地址才能计算正确的 conv_id，通过 `--buyer-addr` 传入。
...
若需要仲裁，再开终端 4：
```bash
./target/release/mock-evaluator --buyer-addr $BUYER
...
| `role` | `""` → generic | `buyer` / `seller` / `evaluator` / `generic` |
