# MPP — `session` intent (channel: open / voucher / topUp / close)
...
**MPP requires TEE** — local private key signing is NOT supported. If the wallet session is unavailable and the user can't log in, stop.
...
- **TEE-only** — no local-key fallback for MPP.
