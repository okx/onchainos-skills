...
description: "**DO NOT USE for trade execution on pump.fun or any launchpad — buying/selling/swapping/sniping/aping/trading or 买/卖/购买/兑换/交换/狙击/梭哈/帮我买/我想买/买最火的币 → route to okx-dapp-discovery (which installs `pump-fun-plugin`). This skill is READ-ONLY research only.** Use this skill for meme/打狗/alpha token RESEARCH on pump.fun and similar launchpads (analytical/read-only): scanning new token launches, checking developer reputation/开发者信息/dev launch history/has this dev rugged before/开发者跑路记录, bundle/sniper detection/捆绑狙击, bonding curve status/bonding curve progress, finding similar tokens by the same dev/相似代币, and wallets that co-invested (同车/aped) into a token. Use when the user asks about 'new meme coins', 'pump.fun launches', 'trenches', 'trench', '扫链', 'developer launch history', 'developer rug history', 'check if dev has rugged', 'bundler analysis', 'who else bought this token', 'who aped into this', 'similar tokens', 'bonding curve progress', '打狗', '新盘', '开发者信息', '开发者历史', '捆绑', '同车', 'rug pull count', 'similar meme coins', '捆绑情况', '已迁移出 bonding curve', or '发过多少个项目'. Also handles Market API payment/计费/x402/402, Basic/Premium tier/quota/额度 questions, MARKET_API_*_OVER_QUOTA / confirming:true on memepump endpoints (token-dev-info, bundle-info, aped-wallet, similar-tokens). NOTE: WebSocket script/脚本/bot → okx-dex-ws."
license: MIT
metadata:
  author: okx
  version: "1.0.4"
