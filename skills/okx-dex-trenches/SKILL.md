...
description: "Use this skill for meme/打狗/alpha token research on pump.fun and similar launchpads: scanning new token launches, checking developer reputation/开发者信息 and past rug pull history/dev launch history/has this dev rugged before/开发者跑路记录, bundle/sniper detection/捆绑狙击/帮我查一下这个meme币的捆绑情况, bonding curve status/bonding curve progress/这个币的bonding curve进度怎么样, finding similar tokens by the same dev/show me tokens similar to this meme coin/相似代币, and wallets that co-invested (同车/aped) into a token/who else bought this token/which wallets co-invested/aped into this token. Use when the user asks about 'new meme coins', 'pump.fun launches', 'trenches', 'trench', 'scan trenches/扫链', 'check dev reputation', 'developer launch history', 'developer rug history', 'check if dev has rugged', 'bundler analysis', 'who else bought this token', 'who aped into this', 'similar tokens', '打狗', '新盘', '开发者信息', '开发者历史', '捆绑', or '同车'. Do NOT use for market-wide whale/smart-money signals — use okx-dex-signal. Do NOT use for per-token holder distribution or honeypot checks — use okx-dex-token."
license: MIT
metadata:
  author: okx
  version: "1.0.4"
