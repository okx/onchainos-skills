...
- **Step 2 · Service — three sub-steps** (collect name + type first; the service type + pricing model decide **how many description parts** to collect — A2MCP always 4; A2A per-call 3 / subscription 2 — so **description comes LAST** in Step 2c; a user who sends everything at once is fine — just proceed). Present each sub-step warmly and scannably: a short numbered list, no `Q1:` jargon. Any example text is illustrative only — use the user's own reply (§Fields-from-user).
  ...
  - **Step 2c · description (ONE message — the Step-2a serviceType branches FIRST: A2MCP always uses the four-part request description regardless of pricing ("per-call" does NOT make it an ordinary A2A service); only A2A branches by the Step-2b pricing model (per-call 3 parts / subscription 2 parts). Show ONLY the matching set. Put each part on its own line, prefixed `1.` / `2.` / `3.` (/ `4.` for A2MCP)):**
    - **A2MCP = API service → request description (all four parts, BLOCKING at §4).** Show the A2MCP request-description prompt (NOT the ordinary-service or trading-signal prompts). Each part on its own line, prefixed `1.`/`2.`/`3.`/`4.`:
    ...
    - **Subscription-priced = trading-signal service** (A2A monthly — pick **2** or **3**) → collect **two parts** — the core-capability summary and the delivery note (omit "what the user must provide": a subscription auto-delivers, so there is nothing to submit per request), prefixed `1.` / `2.`:
      1. **core-capability summary** — what it does + who it's for; *recommended:* name the markets/venues it covers explicitly (**any market is allowed** — DEX, a centralized exchange, Polymarket, Hyperliquid, etc.; list only the ones actually supported). There is **no market whitelist** — never reject or narrow the user's chosen market.
      2. **delivery note** — delivered as structured signals + whether copy-trading is supported + one concrete signal example that starts with the full market name (prefer the full name over an abbreviation like "HL" for clarity) referencing one of the markets declared in part 1 (**whichever market the user chose** — any market is fine). **Always show the user this illustrative signal-example format in the prompt so they know the expected shape** (display hint only — do NOT store it verbatim; the user supplies their own): `DEX Signal: X Layer | $TOKEN (0x12…ab) | BUY | 0.042-0.045 | Slippage ≤1% | Position 5% | Valid within 24h`.
    - **All cases (A2MCP, A2A per-call, A2A subscription):** each part on its own line; total ≤1000 CJK by **East-Asian display width** (CJK = 2, ASCII = 1) — no per-part length limit. Prefer no example prompts, no GitHub/wallet links, no tech-stack/infra details, no disclaimers, and no profit/return guarantees (e.g. "guaranteed profit" / "double your money").
    - **At COLLECTION time, record the service description verbatim and advance — never a hard gate here** (content only; everything else still gates — §4 step 1). If the user omits, contradicts, or asks to skip / keep any of it as-is (in any language), **record it verbatim and advance** — never refuse to proceed at collection, never present it as mandatory here. The gating happens later at §4:
      - **A URL / test-marker and an empty description BLOCK at §4** via `validate-listing` (`pass:false`) for every service type; they are corrected, not declined.
      - **Over-length (total > 1000 CJK) and profit-guarantee wording are advisory only at §4** — `validate-listing` reports them as `severity:"suggest"` (does NOT fail `pass`); surface as a suggestion, never decline or present as mandatory. (The length cap is A2A-only; the profit-guarantee suggestion applies to every service type.)
      - **Paragraph count is NEVER validated** — the 3-part / 2-part layout above is collection guidance only. `validate-listing` has no paragraph-count rule, so a non-empty description of any shape passes, and subscription and per-call services are validated identically. Never tell the user a paragraph count is required.
      - **A2MCP request-description completeness is a hard block at §4** — §4 blocks until all four items are present.
...
4. **Semantic checks the CLI cannot do — always run, regardless of `pass:true`** (merge into step 2's findings list). Check, by meaning:
   ...
   - **Description structure (A2A advisory)** — for A2A services, each part on its own line, covering the content the §3 Step-2c prompt asked for, and leaking no tech-stack / infra names or legal disclaimers. **If it doesn't, surface as a suggestion — do NOT block, and NEVER present it as a mandatory requirement.** Judge the *content*, not a paragraph count: the CLI has no paragraph-count rule and neither should you, so never flag a description merely for having 1, 2, 3, or more paragraphs, and never treat the subscription / per-call split as a structural requirement.
   - **Subscription (= trading-signal) services ONLY** (non-subscription/ordinary services SKIP this entirely): *recommended* — the core-capability part should declare every covered market it supports (**any market/venue is allowed — there is NO market whitelist; never restrict the user to DEX / Polymarket / Hyperliquid or reject a centralized-exchange market**), and the delivery note should carry a concrete signal example that starts with the full market name (prefer the full name over an abbreviation like "HL" for clarity) referencing one of the markets declared in the core-capability part. If it doesn't, **surface it as a suggestion on the card — do NOT block, and NEVER present it as a mandatory requirement.** (This mirrors the CLI, which reports the A2A total-length `D2` and the profit-guarantee `D9` as `severity:"suggest"` — advisory; the URL `D6` and test-marker `U1` stay `severity:"block"`.)
   - **Profit / return guarantee — any language.** The CLI's **D9** is a deterministic backstop for common guarantee phrases in Chinese and English (hardcoded), emitted as `severity:"suggest"` — surface any D9 finding as a suggestion. Its hardcoded list cannot cover every language, so you SHOULD **additionally** flag, by meaning, any profit / return / no-loss guarantee in **ANY** language even when D9 did not (e.g. "guaranteed profit" / "no-loss" / "double your money" / "blow up your gains") — as the same suggestion, recommending the user describe the capability, not a promised outcome. **This is advisory, not a block.** If the user keeps the guarantee wording after seeing the suggestion, proceed.
   - **A2MCP request-description completeness (BLOCKING — `serviceType == "A2MCP"` only).** **Contract-address exemption: if the `serviceDescription` contains a contract address (`0x` followed by 40 hex chars), skip this check entirely and pass.** Otherwise, for each A2MCP service, verify by semantic judgment that its `serviceDescription` carries all four, in order: (1) **what the service does**, (2) the **parameter spec** — all key parameters on ONE line, separated by `；`, each in the strict format `<name>（<type>，必填/可选）：<含义>` (identity-invariants.md §A2MCP `serviceDescription` structure), (3) the **request method** (`POST`/`GET` or tool name), (4) a **working CURL example** using the real endpoint. All four present by meaning → pass. Any missing → **block the flow**: do NOT advance to the §7 confirmation card; show the rejection reason + user suggestion from identity-invariants.md §A2MCP `serviceDescription` structure (no copyable template), then re-collect that description. This is a hard gate (the PRD mandates a block) — unlike the advisory A2A content checks above. Register and update (identity-update.md §4) use the same rule and copy. Never block solely because not every parameter is enumerated (overflow tie-break — §A2MCP `serviceDescription` structure). A present-but-loosely-worded per-parameter spec is **proactively normalized into the strict format and confirmed with the user** (§A2MCP `serviceDescription` structure), NOT blocked — the block fires only for an entirely-missing item.
