...
- **Step 2 · Service — three sub-steps** (collect name + type first; the pricing model picked in Step 2b decides **how many description parts** to collect, so **description comes LAST** in Step 2c; a user who sends everything at once is fine — just proceed). Present each sub-step warmly and scannably: a short numbered list, no `Q1:` jargon. Any example text is illustrative only — use the user's own reply (§Fields-from-user).
  ...
  - **Step 2c · description (ONE message — the Step-2b pricing model decides BOTH the part count AND which prompt set to show. Show ONLY the matching set. Put each part on its own line, prefixed `1.` / `2.` / `3.`):**
    - **A2MCP = API service → request description (all three parts, BLOCKING at §4).** Show the A2MCP request-description prompt (NOT the ordinary-service or trading-signal prompts). Each part on its own line, prefixed `1.`/`2.`/`3.`:
      1. **service description** — what the service does (its function / purpose).
      2. **parameter specification** — ALL key parameters on ONE line, separated by `；`, each in the strict format `<name>（<type>，必填/可选）：<含义>` (optional param: append `，<默认值>` to the meaning); key params suffice (overflow rule in §A2MCP `serviceDescription` structure). If the user's input is present but not in this format, proactively rewrite it into the strict one-line `；`-separated format and confirm (or let them correct it) with the user before storing — see identity-invariants.md §A2MCP `serviceDescription` structure.
      3. **request method** — how to call it: **only** an HTTP-protocol verb (`POST` / `GET` / `PUT` / `PATCH` / `DELETE` / …) **or** a bare MCP tool name. **No URL, no path, no other text** — the endpoint already lives in the separate `endpoint` (接口地址) field. If the user pastes a URL or path here (e.g. `POST https://…/okx/scan` or `/okx/scan`), keep only the verb (or tool name) and drop the rest → store `POST` (identity-invariants.md §A2MCP `serviceDescription` structure — *Request-method — verb / tool-name only* bullet).
      Prefix each stored line with its bracketed label — `[服务描述]` / `[入参说明]` / `[请求方式]` (localized): keep the user's label if they typed one, supplement it if they didn't (identity-invariants.md §A2MCP `serviceDescription` structure — *Label prefix* bullet). **Do NOT show any copyable fill template** — the inline three-part prompt above is the only guidance; reformat whatever the user gives into `1./2./3.` for storage (identity-invariants.md §A2MCP `serviceDescription` structure holds the §4 block copy).
    - **A2A per-call = ordinary service** (pick **1**) → collect **all three parts**, using the **ordinary-service** prompt ONLY (do NOT show trading-signal hints — the market-declaration / signal-example requirements do NOT apply here):
    ...
    - **Subscription-priced = trading-signal service** (A2A monthly — pick **2** or **3**) → collect **two parts** — the core-capability summary and the delivery note (omit "what the user must provide": a subscription auto-delivers, so there is nothing to submit per request), prefixed `1.` / `2.`:
      1. **core-capability summary** — what it does + who it's for; *recommended:* name each covered market explicitly, using only **DEX / Polymarket / Hyperliquid** (list only the ones actually supported).
    ...
    - **All cases (A2MCP, A2A per-call, A2A subscription):** each part on its own line; each part ≤200 CJK / total ≤600 CJK by **East-Asian display width** (CJK = 2, ASCII = 1). Prefer no example prompts, no GitHub/wallet links, no tech-stack/infra details, no disclaimers, and no profit/return guarantees (e.g. "guaranteed profit" / "double your money").
    - **At COLLECTION time, record the service description verbatim and advance — never a hard gate here** (content only; everything else still gates — §4 step 1). If the user omits, contradicts, or asks to skip / keep any of it as-is (in any language), **record it verbatim and advance** — never refuse to proceed at collection, never present it as mandatory here. The gating happens later at §4:
      - **Structure/length (wrong paragraph count / over-length) and prohibited content (URL / 0x / profit-guarantee / test-marker) BLOCK at §4** via `validate-listing` (`pass:false`); they are corrected, not declined.
      - **A2MCP request-description completeness is a hard block at §4** — §4 blocks until all three items are present.
      - Only the residual **semantic** quality (unclear paragraphs, tech-stack/disclaimer/sample-prompt leak, subscription market + signal example) is handled by optimize-and-confirm at §4, where the user may accept the drafted rewrite or keep their own.
...
1. **Call once, on the full set.** **Hard precondition (SKILL §Gates Service-collection): unless the user has explicitly chosen Done in the §3 Step-2 add-another prompt (1. Add another / 2. Done), you MUST NOT call `validate-listing` — no matter how complete the fields look.** A single batched message carrying every field for one service does NOT satisfy this; ask the add-another prompt and wait for the Done choice first. Only after the user picks *Done* in §3 Step 2, run `validate-listing --role asp --name … --description … --service '[… all collected services …]'` a single time. Returns `{ pass, findings[{field, code, severity, message}] }` where every finding is `severity:"block"` (there is no advisory tier in the CLI any more). `code` is a fine-grained diagnostic (`U1`/`N1`/`D6`…) for grouping only (never shown to the user); `message` is the rule's single unified 提示文案 and the only rule-message field exposed (the internal canonical rule id is deliberately not serialized). `field` uses dot-notation (e.g. `service[0].fee`, `service[1].name`). **Service-description structure/length and prohibited content — URL / 0x / profit-guarantee / test-marker — now BLOCK like every other field** (per ASP重构-身份侧校验; the old `service[i].servicedescription` = `"warn"` advisory tier is retired). The only service-description checks that remain advisory are the purely SEMANTIC ones the CLI cannot decide — those live in step 4 below, not in the CLI output.
2. **Render the findings card.** Always run the semantic checks in step 4 first and merge with the CLI findings. **A2MCP request-description block:** if step 4's A2MCP three-item check fails for any A2MCP service, the flow is **blocked** — show that service's rejection reason + user suggestion (identity-invariants.md §A2MCP `serviceDescription` structure — no copyable template) and re-collect its description; do NOT advance to §7 until every A2MCP service passes, regardless of the apply/revise choice below (which governs only how the collected corrections are gathered). Only when `pass:true` AND no semantic issues AND every A2MCP service's request description is complete → say it passed and go straight to §7. Otherwise render the findings inline on their field rows, mapping by the dotted `finding.field` to its card row (`service[0].fee` → Service [1]'s Fee row, `service[1].*` → Service [2]'s rows, `name` → the identity Name row). **Render each finding's translated `message` — de-duplicated by (`field`, `message`) so one unified sentence shows once per field** even when several `code`s produced the same `message` (never print the `code` token itself). Surface a `(test)` marker on the name row if present. **A CLI finding (`pass:false`) is a BLOCK, not a suggestion**: its `message` already tells the user what to change, and you MUST NOT advance to §7 until the offending field is corrected and (on the next collect) re-validates clean. The apply/revise choice below governs how the correction is collected; it does not turn a block into an optional tip. **The field values are unchanged at display time — do NOT silently apply any change yet.**
3. **Confirmation is mandatory — never apply a change before the user chooses.** After showing the card (each flagged field carrying its unified `message` + any skill-semantic optimize-and-confirm draft), ask once how to proceed — exactly TWO numbered choices (localized). Do NOT re-run `validate-listing`:
   > 1. Use the corrected version — I'll fill each flagged field with the fix drafted above, then redraw the card for you to review.
   > 2. I'll revise it myself — tell me the new value(s).
   - On **1**: this choice **is** the user's confirmation for the whole batch. For each flagged field, apply the drafted correction — for a CLI-block finding, the compliant value you draft FROM its `message` (e.g. trim to ≤ 500 chars, remove the URL/0x/test-marker, fix the paragraph count); for a skill-semantic finding, the ✏️ draft you proposed in step 4 — then redraw the card with the corrected values. Apply **once** — do not iterate. Every field you drafted stays flagged ` ✏️ drafted from your words — please review`.
   - On **2**: collect the user's replacement value(s) for the flagged field(s) and redraw the card.
   Either way, the corrected values still flow into the §7 confirmation card — **nothing is written on-chain until the user confirms there (Reply 1)**. **`validate-listing` has already run its single pass — never call it again** (`activate` does NOT re-run QA; listing QA happens only here at register and at update). Never apply a correction before the user picks; never silently auto-correct. **A CLI-block field must actually be corrected — you may not carry an unresolved block onto the §7 card.**
4. **Semantic checks the CLI cannot do — always run, regardless of `pass:true`** (merge into step 2's findings list). These are the doc's **skill 规则** rows (internal ids, never shown: the agent-name, service-name, and description-quality semantic checks). **Uniform handling for every skill-rule hit (要求②):** (a) show the user that rule's canonical 提示文案 (localized, message text only — never a rule number) as the reason; (b) **proactively optimize one improved version** of the offending field, following the rule's guidance; (c) present it as a **draft** flagged ` ✏️ drafted from your words — please review` (identity-invariants.md §Confirmation requirement) and **ask the user to confirm it or supply their own** — never silently apply, never just drop a passive tip. Check, by meaning:
   - **Service name** — a descriptive noun-phrase that conveys what the service does, not a vague token like a single letter "Q". On hit: show its 提示文案 (message text only), draft a clearer descriptive name, and ask the user to confirm/replace.
   - **Agent name** — a neutral brand, not a personal label (Alice, Account2), and NOT containing a celebrity / public-figure name as a substring (even prefixed/suffixed — Trump, Musk, CZ, 马斯克, 马云). On hit: show its 提示文案 (message text only), draft a neutral brand-name alternative, and ask the user to confirm a compliant name. Do not carry a celebrity/personal name onto the §7 card. Per `identity-invariants.md` §Fields-from-user.
   - **Description SEMANTIC quality** — the paragraphs read clearly and the description leaks NO tech-stack / infra names, legal disclaimers, or sample prompts; and, **for subscription (= trading-signal) services ONLY** (ordinary services SKIP this), the core-capability part declares every covered market using ONLY DEX / Polymarket / Hyperliquid and the delivery note carries a concrete signal example that starts with the full market name (never an abbreviation like "HL") and references only a declared market. On hit: show its 提示文案 (message text only), draft a rewritten description (3 parts non-subscription / 2 parts subscription, tech-architecture / disclaimers / sample prompts removed; market + signal example added for subscription signal services), and ask the user to confirm or revise.
     - **NOTE — structure/length and prohibited content are NOT here.** Wrong paragraph count / over-length, and URL / 0x / profit-guarantee / test-marker, are now **CLI blocks** surfaced by `validate-listing` in step 2 (`pass:false`) — do not re-classify them as advisory. This semantic check is only the residual judgment the CLI cannot make. The former "any-language profit-guarantee" advisory belongs here: the CLI's deterministic phrase list (Chinese + English) now BLOCKS as prohibited content, and you still additionally flag, by meaning, any profit / return / no-loss guarantee in ANY language the CLI missed — via the same optimize-and-confirm handling (draft a version that describes the capability, not a promised outcome).
   - **A2MCP request-description completeness (BLOCKING — `serviceType == "A2MCP"` only).** For each A2MCP service, verify by semantic judgment that its `serviceDescription` carries all three: (1) **what the service does**, (2) **per-parameter spec** — all key parameters on one line, separated by `；`, each in the strict format `<name>（<type>，必填/可选）：<含义>`, (3) the **request method** (`POST`/`GET` or tool name) (identity-invariants.md §A2MCP `serviceDescription` structure). All three present by meaning → pass. Any missing → **block the flow**: do NOT advance to the §7 confirmation card; show the rejection reason + user suggestion from identity-invariants.md §A2MCP `serviceDescription` structure (no copyable template), then re-collect that description. This is a hard gate (the PRD mandates a block) — like the CLI's structure/length and prohibited-content blocks, and distinct from the semantic checks above (optimize-and-confirm, not a silent tip). Register and update (identity-update.md §4) use the same rule and copy. Never block solely because not every parameter is enumerated (overflow tie-break — §A2MCP `serviceDescription` structure). A present-but-loosely-worded per-parameter spec is **proactively normalized into the strict format and confirmed with the user** (§A2MCP `serviceDescription` structure), NOT blocked — the block fires only for an entirely-missing item.
