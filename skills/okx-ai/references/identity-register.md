...
4. **Semantic checks the CLI cannot do — always run, regardless of `pass:true`** (merge into step 2's findings list). Check, by meaning:
   ...
   - **Description structure (A2A advisory)** — for A2A services, matches the pricing-based part rule in §3 Step 2c (non-subscription = 3 parts / subscription = 2 parts, each part on its own line) and leaks no tech-stack / infra names or legal disclaimers. **If it doesn't, surface as a suggestion — do NOT block, and NEVER present it as a mandatory requirement** (service-description content is advisory, per §3 Step 2c; mirrors the CLI, which reports it as `severity:"warn"`).
   - **Subscription (= trading-signal) services ONLY** (non-subscription/ordinary services SKIP this entirely): *recommended* — the core-capability part should declare every covered market it supports (**any market/venue is allowed — there is NO market whitelist; never restrict the user to DEX / Polymarket / Hyperliquid or reject a centralized-exchange market**), and the delivery note should carry a concrete signal example that starts with the full market name (prefer the full name over an abbreviation like "HL" for clarity) referencing one of the markets declared in the core-capability part. If it doesn't, **surface it as a suggestion on the card — do NOT block, and NEVER present it as a mandatory requirement.** (This mirrors the CLI, where every `service[i].servicedescription` finding is `severity:"warn"`.)
   - **Profit / return guarantee — any language.** The CLI's **D9** is a deterministic backstop for common guarantee phrases in Chinese and English (hardcoded), now emitted as `severity:"warn"` — surface any D9 finding as a suggestion. Its list cannot cover every language, so you SHOULD **additionally** flag, by meaning, any profit / return / no-loss guarantee in **ANY** language even when D9 did not (e.g. "guaranteed profit" / "no-loss" / "double your money" / "blow up your gains") — as the same suggestion, recommending the user describe the capability, not a promised outcome. **This is advisory, not a block** (product decision — the service-description content is warning-only). If the user keeps guarantee wording after seeing the suggestion, proceed.
   - **A2MCP request-description completeness (BLOCKING — `serviceType == "A2MCP"` only).** For each A2MCP service, verify by semantic judgment that its `serviceDescription` carries all three, in order: (1) **what the service does**, (2) the **parameter spec** — all key parameters on ONE line, separated by `；`, each in the strict format `<name>（<type>，必填/可选）：<含义>` (identity-invariants.md §A2MCP `serviceDescription` structure), (3) the **request method** (`POST`/`GET` or tool name). All three present by meaning → pass. Any missing → **block the flow**: do NOT advance to the §7 confirmation card; show the rejection reason + user suggestion from identity-invariants.md §A2MCP `serviceDescription` structure (no copyable template), then re-collect that description. This is a hard gate (the PRD mandates a block) — unlike the advisory A2A content checks above. Register and update (identity-update.md §4) use the same rule and copy. Never block solely because not every parameter is enumerated (overflow tie-break — §A2MCP `serviceDescription` structure). A present-but-loosely-worded per-parameter spec is **proactively normalized into the strict format and confirmed with the user** (§A2MCP `serviceDescription` structure), NOT blocked — the block fires only for an entirely-missing item.
