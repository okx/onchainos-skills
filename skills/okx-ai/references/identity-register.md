...
4. **Semantic checks the CLI cannot do — always run, regardless of `pass:true`** (merge into step 2's findings list). Check, by meaning:
   ...
   - **Description quality (A2A advisory)** — check ONLY these service-description parts, by meaning: `1.` core capability summary (**required**) = capability points + who it's for (+ for a signal service, what kind of trading signals it pushes); `2.` what the user must provide (**optional**) = materials such as wallet address / amount / chain; `3.` delivery note (**optional**) = delivery format, and for a trading-signal service copy-trading notes may be included. **If the required part `1.` is missing or unclear, surface it as a suggestion — do NOT block, and NEVER present it as a mandatory requirement.** Never add other service-description semantic suggestions. Judge the *content*, not a paragraph count: the CLI has no paragraph-count rule and neither should you, so never flag a description merely for having 1, 2, 3, or more paragraphs, never treat the subscription / per-call split as a structural requirement, and **never ask for a missing part `2.` or part `3.` — both are optional**. (Same split as step 1 above: `D2` suggests, `D6`/`U1` block.)
