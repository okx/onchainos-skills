...
- **Step 2 · Service — three sub-steps** (collect name + type first; the service type + pricing model decide **how many description parts** to collect — A2MCP always 4; A2A per-call 3 / subscription 2 — so **description comes LAST** in Step 2c; a user who sends everything at once is fine — just proceed). Present each sub-step warmly and scannably: a short numbered list, no `Q1:` jargon. Any example text is illustrative only — use the user's own reply (§Fields-from-user).
  ...
  - **Step 2c · description (ONE message — branch on the Step-2a serviceType ONLY: A2MCP uses the four-part request description regardless of pricing ("per-call" does NOT make it an ordinary A2A service); every A2A uses the SAME three-part prompt regardless of the Step-2b pricing model — there is no per-call / subscription variant. Show ONLY the matching set. Put each part on its own line. **A2MCP keeps its `1.`/`2.`/`3.`/`4.` numbering + bracketed labels (they carry meaning for the request description); A2A needs NO numbering prefix** — one line per part is enough, and numbering is optional when several parts are present):**
    ...
    - **A2A = agent to agent → three parts, the SAME prompt for per-call and subscription pricing alike** (do NOT branch on the Step-2b pick, do NOT show any market-declaration or signal-example hint — those requirements no longer exist). Part `1.` is required; parts `2.` and `3.` are optional — if the user leaves either out, record what they gave and advance:
      ...
      2. **what the user must provide (optional)** — list the materials the user has to supply, e.g. `1. 钱包地址 2. 金额 3. 链`.
      3. **delivery note (optional)** — the delivery format; for a trading-signal service you may add how copy-trading works.
    - **All cases (A2MCP + A2A):** each part on its own line; total ≤1000 CJK by **East-Asian display width** (CJK = 2, ASCII = 1) — no per-part length limit. **No links of any kind** — every URL is hard-blocked at §4 (this supersedes the old "prefer no GitHub / wallet links" wording; a wallet or contract address in the text is fine and blocks nothing). Also prefer no example prompts, no tech-stack/infra details, no disclaimers, and no profit/return guarantees (e.g. "guaranteed profit" / "double your money") — those four are advisory, not blocking.
      - **A2MCP exception:** the request example on line `4.` MUST carry the real endpoint URL, so an A2MCP description necessarily contains one. Collect it as asked.
    - **At COLLECTION time, record the service description verbatim and advance — never a hard gate here** (content only; everything else still gates — §4 step 1). If the user omits, contradicts, or asks to skip / keep any of it as-is (in any language), **record it verbatim and advance** — never refuse to proceed at collection, never present it as mandatory here. The gating happens later at §4:
      ...
      - Only the residual **semantic** quality (unclear wording, tech-stack/disclaimer/sample-prompt leak) is handled by optimize-and-confirm at §4, where the user may accept the drafted rewrite or keep their own.
...
4. **Semantic checks the CLI cannot do — always run, regardless of `pass:true`** (merge into step 2's findings list). Check, by meaning:
   ...
   - **Description quality (A2A advisory)** — for A2A services, the required part `1.` actually states a capability and who it's for, each part is on its own line, and nothing leaks tech-stack / infra names or legal disclaimers. **If it doesn't, surface as a suggestion — do NOT block, and NEVER present it as a mandatory requirement.** Judge the *content*, not a paragraph count: the CLI has no paragraph-count rule and neither should you, so never flag a description merely for having 1, 2, 3, or more paragraphs, never treat the subscription / per-call split as a structural requirement, and **never ask for a missing part `2.` (what the user provides) or part `3.` (delivery note) — both are optional**. (This mirrors the CLI, which reports the A2A total-length `D2` and the profit-guarantee `D9` as `severity:"suggest"` — advisory; the URL `D6` and test-marker `U1` stay `severity:"block"`.)
   - **No market or signal-example requirement.** A2A services — including subscription-priced signal services — have **no** declared-market rule and **no** signal-example rule. Never ask the user to enumerate covered markets/venues, never ask for a signal example or a specific signal format, and never surface either as a suggestion. Any market the user names is fine and needs no validation.
