...
- **Step 2 · Service — three sub-steps** (collect name + type first; the pricing model picked in Step 2b decides **how many description parts** to collect, so **description comes LAST** in Step 2c; a user who sends everything at once is fine — just proceed). Present each sub-step warmly and scannably: a short numbered list, no `Q1:` jargon. Any example text is illustrative only — use the user's own reply (§Fields-from-user).
  ...
  - **Step 2c · description (ONE message — the Step-2b pricing model decides BOTH the part count AND which prompt set to show. Show ONLY the matching set. Put each part on its own line, prefixed `1.` / `2.` / `3.`):**
    - **A2MCP = API service → request description (all three parts, BLOCKING at §4).** Show the A2MCP request-description prompt (NOT the ordinary-service or trading-signal prompts). Each part on its own line, prefixed `1.`/`2.`/`3.`:
      ...
      2. **request method** — how to call it: `POST`, `GET`, or the MCP tool name.
      3. **parameter specification** — each key parameter on its own line in the strict format `<name>（<type>，必填/可选）：<含义>` (optional param: append `，<默认值>`); key params suffice (overflow rule in §A2MCP `serviceDescription` structure). If the user's input is present but not in this format, proactively rewrite it into the strict per-line format and confirm (or let them correct it) with the user before storing — see identity-invariants.md §A2MCP `serviceDescription` structure.
...
4. **Semantic checks the CLI cannot do — always run, regardless of `pass:true`** (merge into step 2's findings list). Check, by meaning:
   ...
   - **A2MCP request-description completeness (BLOCKING — `serviceType == "A2MCP"` only).** For each A2MCP service, verify by semantic judgment that its `serviceDescription` carries all three: (1) **what the service does**, (2) the **request method** (`POST`/`GET` or tool name), (3) **per-parameter spec** — each key parameter in the strict format `<name>（<type>，必填/可选）：<含义>` (identity-invariants.md §A2MCP `serviceDescription` structure). All three present by meaning → pass. Any missing → **block the flow**: do NOT advance to the §7 confirmation card; show the rejection reason + user suggestion + copyable template from identity-invariants.md §A2MCP `serviceDescription` structure, then re-collect that description. This is a hard gate (the PRD mandates a block) — unlike the advisory A2A content checks above. Register and update (identity-update.md §4) use the same rule and copy. Never block solely because not every parameter is enumerated (overflow tie-break — §A2MCP `serviceDescription` structure). A present-but-loosely-worded per-parameter spec is **proactively normalized into the strict format and confirmed with the user** (§A2MCP `serviceDescription` structure), NOT blocked — the block fires only for an entirely-missing item.
