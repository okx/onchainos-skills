...
4. **Semantic checks the CLI cannot do — always run, regardless of `pass:true`** (merge into step 2's findings list). Check, by meaning:
   ...
   - **Description quality (A2A semantic — FE-23 advisory)** — the pricing-based part count / structure and total length of an A2A description are already enforced by the CLI (FE-21, `severity:"block"`; corrected in step 2 — do NOT re-check them here). Here check only what the CLI cannot: each part is clearly worded and leaks no tech-stack / infra names or legal disclaimers. **If it doesn't, surface as a suggestion — do NOT block, and NEVER present it as a mandatory requirement.**
   - **Subscription (= trading-signal) services ONLY** (non-subscription/ordinary services SKIP this entirely): *recommended* — the core-capability part should declare every covered market it supports (**any market/venue is allowed — there is NO market whitelist; never restrict the user to DEX / Polymarket / Hyperliquid or reject a centralized-exchange market**), and the delivery note should carry a concrete signal example that starts with the full market name (prefer the full name over an abbreviation like "HL" for clarity) referencing one of the markets declared in the core-capability part. If it doesn't, **surface it as a suggestion on the card — do NOT block, and NEVER present it as a mandatory requirement.** (FE-23 skill-layer semantic check — advisory; separate from the CLI's blocking FE-21/FE-22 findings.)
   - **Profit / return guarantee — any language.** The CLI's **D9** (FE-22) is a deterministic backstop for common guarantee phrases in Chinese and English (hardcoded) and is `severity:"block"` — a D9 finding must be corrected in step 2 like any other block; never soften it into a tip. Its list cannot cover every language, so you SHOULD **additionally** flag, by meaning, any profit / return / no-loss guarantee in **ANY** language that D9 did not catch (e.g. "guaranteed profit" / "no-loss" / "double your money" / "blow up your gains") — as a suggestion, recommending the user describe the capability, not a promised outcome. **This semantic extension is advisory, not a block** (product decision — only the CLI's hardcoded D9 phrase list blocks). If the user keeps the guarantee wording after seeing the suggestion and D9 did not fire, proceed.
