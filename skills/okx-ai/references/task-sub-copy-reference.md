...
|---|---|---|---|
| 1 | `sub_created` | user | Creation-success notice after authoritative Active validation, A2A session establishment/restoration, and best-effort forwarding of every pending attachment. `trialType=1` → "[Trial Started] Your free trial is active ({trialStartTime}–{trialEndTime}). After it ends, {tokenAmount} {tokenSymbol} will be auto-charged on {trialEndTime} to convert to a paid subscription (attempted once, within the final hour before the trial ends — it will not retry if missed)." Otherwise → "[Subscribed] Job {jobId} (subscribing to {jobTitle}) is on-chain, status: Active. Current period {subStartTime}–{subEndTime}. First charge of {tokenAmount} {tokenSymbol} completed. Auto-renew is on; next charge date: {subEndTime}." This is not an ASP acceptance notice. |
