...
🛑 This Watch handoff is the **last non-Watch action in the creation flow** — once entered, `watch-core.md` owns the rest of the turn, including every required dispatch and re-entry. Do not run unrelated creation commands after the handoff, and do not confuse "last creation action" with permission to stop after the first watch result. On `sub_created`, the CLI fetches authoritative task detail, establishes or restores the designated ASP session, and independently forwards every pending attachment; one attachment failure never blocks the remaining files or the notification. The agent then renders the fixed trial/non-trial subscription-success copy and resumes scoped watch. It does NOT re-scan the description for DApp names, auto-install a plugin, pre-select a tool, or execute the Service Guide. Local tool preparation occurs only during the matching `serviceGuide` setup step. Authentication and trading availability are decided only by the final target command. A failed delivery remains visible and is never auto-replayed, while future deliveries continue normally.
