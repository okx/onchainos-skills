...
| `sub_asp_agree` / `sub_reject_refund_notify` | Run `next-action` and render its returned content. Fresh `FAILED(9)` detail plus matching ASP, Service, full original-token amount, and a valid Tx Hash from this dedicated final-refund event produce `[Refund Settled]` / `[Auto-Refund Settled]` plus a **terminal hint**; follow that hint and close the session. Missing, stale, or mismatched proof produces `[Refund Settlement Detail Incomplete]` with no terminal hint: do not claim completion, do not call a client-side claim, and keep the scoped watch active. **end turn**. |
| `sub_complete_notify` | Route the structured result through [`task-action-routing.md`](task-action-routing.md). |
| `sub_failed_notify` | Fresh Buyer ownership and Failed(9) are required, but this event does not authoritatively distinguish a charge failure from refund settlement. Render the `[Trial Ended]` / `[Subscription Ended]` lifecycle copy plus the returned settlement warning. Do not follow a terminal hint or clean up; run read-only `refund-prepare` and keep watching unless it proves `reason=refund_confirmed`. **end turn**. |
