...
| Event | Action |
|---|---|
| `sub_created` / `sub_asp_selected` / `sub_trial_into_active` / `sub_renew` | `next-action --role user --agentId <yours> --message '<envelope>'` → render the returned `Content:` per the **`sub_*` language rule** below → `onchainos agent user-notify --content "<rendered>"` → **end turn**. `sub_created` is sent to both Buyer and ASP after create-subscribe is confirmed; it requires `subStatus/status=CREATED(0)`, owns session establishment/restoration and pending-attachment forwarding, and tells the Buyer that ASP acceptance is pending. `sub_asp_selected` requires `ACTIVE(1)` before telling the Buyer that the ASP accepted and service started. A fetch failure, missing status, or mismatched status blocks the event flow. Event fields take precedence for event-specific dates; missing title/payment display fields fall back to the authoritative detail. `sub_open` is obsolete and ignored. |
...
| `sub_asp_agree` / `sub_reject_refund_notify` | Run `next-action` and render its returned content. These indicate an ASP decision or expected auto-refund path, not authoritative backend transaction-result evidence. Subscription `FAILED(9)` is also used for charge failure, and the current event input is caller-supplied; even the name `job_auto_refunded` cannot disambiguate it. `[Refund Settled]` / `[Auto-Refund Settled]` and a **terminal hint** require a future authoritative backend cause/query or server-verifiable result plus fresh matching `FAILED(9)`, Buyer, and full original-token amount. Tx Hash may be unavailable; missing ASP/Service labels affect display only. Until then render `[Refund Settlement Detail Incomplete]` with no terminal hint: do not claim completion, do not call a client-side claim, and keep the scoped watch active. **end turn**. |
| `sub_complete_notify` | Route the structured result through [`task-action-routing.md`](task-action-routing.md). |
| `sub_failed_notify` | Fresh Buyer ownership and Failed(9) are required. This subscription event carries charge-failure semantics and does not prove a refund transition. Render the `[Trial Ended]` / `[Subscription Ended]` lifecycle copy plus the returned refund-cause warning. Do not follow a terminal hint or clean up; run read-only `refund-prepare` and keep watching unless a separate authoritative backend refund-success result makes it return `reason=refund_confirmed`. **end turn**. |
| `sub_close_notify` | Fresh subscription detail must prove Buyer ownership and Closed(7). Show the normal close copy, or the pre-activation ASP-decline copy with `aspRejectReason` verbatim. Current detail does not expose an authoritative close cause, so neither branch proves refund settlement: return no terminal marker/hint and keep the Buyer refund watch/session live. Run read-only `refund-prepare` and follow only its returned actions. |
