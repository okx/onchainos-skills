...
| `sub_asp_agree` / `sub_reject_refund_notify` | Run `next-action`, then apply the settlement and terminal rules in [`user/refund.md`](../../okx-ai-v2/references/a2a/user/refund.md). Render only the returned result; the event itself neither proves settlement nor authorizes a Buyer write. **end turn**. |
| `sub_complete_notify` | Route the structured result through [`task-action-routing.md`](task-action-routing.md). |
| `sub_failed_notify` | Run `next-action` and render its returned read-only result. This event does not independently establish refund or charge-failure cause; follow [`user/refund.md`](../../okx-ai-v2/references/a2a/user/refund.md) and emit no terminal effects unless the structured result provides them. **end turn**. |
| `sub_close_notify` | Fresh subscription detail must prove Buyer ownership and Closed(7). Show the returned close result, preserving `aspRejectReason` verbatim when present. If the result indicates refund reconciliation, follow [`user/refund.md`](../../okx-ai-v2/references/a2a/user/refund.md); Closed alone is not refund settlement. |
...
Use [`user/refund.md`](../../okx-ai-v2/references/a2a/user/refund.md) as the only settlement and
