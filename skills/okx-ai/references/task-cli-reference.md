...
into Funding. A normalized Service with a non-blank `serviceGuide` always includes the CLI-derived
`serviceGuideHash` for that exact Guide. Stable non-Funding phase values are `login_validation`,
