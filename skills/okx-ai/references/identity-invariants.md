...
| `serviceDescription` | yes | numbered parts on separate lines, each prefixed `1.` / `2.` / `3.`. Part count & meaning follow serviceType + pricing model: **A2MCP → 4 parts (request description — see §A2MCP `serviceDescription` structure)**; **A2A non-subscription (per-call) → 3 parts** (`1.` core-capability summary · `2.` what the user must provide · `3.` delivery note); **A2A subscription-priced → 2 parts** (`1.` core-capability summary · `2.` delivery note — "what the user must provide" omitted, since a subscription auto-delivers). Hard limit: total ≤1000 CJK chars (no per-part length limit); no links / profit guarantees / test markers; avoid example prompts / tech-stack / disclaimers. Subscription-priced services (= the trading-signal service, per this skill's convention; non-subscription services are ordinary and skip this): the core-capability part should declare covered markets (any market/venue is allowed — no whitelist) and the delivery note should carry a full-market-name signal example (register §4). **For A2A, structure/length (FE-21: exact part count, ≤1000-CJK total) and prohibited content (FE-22: URL / profit guarantee / test marker) are CLI `severity:"block"` rules — they fail `pass` like every other field; only semantic quality (FE-23: example prompts / tech-stack / disclaimers / market declaration / signal example) stays advisory (register §4). A2MCP uses the blocking request-description check (§A2MCP `serviceDescription` structure).** Length is counted in **East-Asian display width** (CJK = 2, ASCII = 1) |
...
| `2.` | `[Parameter Spec]` — **all** key parameters on **one line**, separated by `;` (full-width `；` for CJK), each in the **strict format** `<name>(<type>, required/optional): <meaning>` (see the *Parameter-spec strict format* bullet below) | What the user must provide (non-sub) / delivery note (sub) |
| `3.` | `[Request Method]` — **only** an HTTP-protocol verb (`POST` / `GET` / `PUT` / `PATCH` / `DELETE` / …) **or** a bare MCP tool name. **No URL, no path, no other text** — the callable address already lives in the separate `endpoint` field (see the *Request-method — verb / tool-name only* bullet below) | Delivery note (non-sub only) |
...
- **Blocking, not advisory.** All four A2MCP items must be present by meaning (not literal keywords). Any missing → the Skill **blocks** the flow at register §4 / update §4 (wherever `validate-listing` runs; `activate` does not re-run QA). A2A's mechanical checks (FE-21 structure/length, FE-22 prohibited content) also block — via the CLI's `validate-listing` findings; the difference is only WHERE the check runs (A2MCP four-item completeness = skill-layer semantic judgment; A2A structure = CLI mechanical rule). Only A2A semantic quality (FE-23) stays advisory. **Contract-address exemption:** if the `serviceDescription` contains a contract address (`0x` followed by 40 hex characters, e.g. `0xAbCd…1234`), skip the four-item completeness check entirely and accept the description as-is.
