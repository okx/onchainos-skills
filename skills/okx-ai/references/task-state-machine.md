...
| `9` | `failed` | `Status::Failed` | One-time: backend chain-projected successful refund transition. Subscription: overloaded refund/terminal-charge-failure state, so refund cause must be disambiguated. | `job_refunded`, `job_auto_refunded`, or subscription failure events |

> ⚠️ **`Status::Failed` (int 9) is task-kind dependent.** For a one-time task, the backend writes this chain-projected state only for successful refund transitions, so fresh matching Refund V2 detail may say "refund completed" even when Tx Hash is unavailable. For a subscription, `FAILED` also represents terminal charge failure. The current event input is caller-supplied, so even the name `job_auto_refunded` cannot disambiguate it; remain fail-closed until an authoritative cause/query or server-verifiable source exists. The Mermaid diagram below uses `refunded` only as the one-time state-machine-friendly name.
>
...
>
> Backend semantics define `job_closed` and `job_auto_refunded` as successful, confirmed transaction-result notifications, not `uopData.executeResult` preflight. They may omit Tx Hash. Because current event input is caller-supplied, re-read fresh task-kind/status/ownership/payment facts; for subscriptions the name is not authoritative cause evidence. Missing, null, or non-boolean `executeResult` must never be treated as preflight success.
>
