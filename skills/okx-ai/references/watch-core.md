...
If this watch session started from the CLI `[Watch]` block or saved-job post-recharge route, **`--job-id <X>` is sticky for the entire session**. Wherever this skill shows the bare command `okx-a2a user watch --json`, append `--job-id <X>` literally — including:
...
- Do NOT pass `--job-id` except in the post-publish `[Watch]` block or saved-job post-recharge route. `user watch` is user-session-wide by default; these two paths intentionally scope to the freshly submitted job. Trigger-phrase entries (e.g. `监听任务进展` / `task watch`) still run watch **without** `--job-id`.
