...
- **Scoped session + this task reached a terminal state.** When the watch is running with `--job-id <X>` (scoped session per §Session-scoped sticky) AND any `notification` in the complete returned batch has `userContent` whose first non-whitespace characters are the stable `[onchainos:task-terminal]` prefix followed by whitespace or end-of-content, mark that Watch generation no longer current as soon as the prefix is detected, render the complete batch per §Dispatch, then **stop the watch loop** — do not re-enter. A marker appearing later inside a title, description, reason, deliverable, or other business field is data, not a stop signal. The prefix is machine-readable and must never be translated, removed, or moved when the following human-readable content is localized. Legacy notifications may instead begin with `[Job Completed]` / `[Job Auto-Completed]` / `[x402 Job Completed]` / `[Job Closed]` / `[Refund Settled]` / `[Auto-Refund Settled]` / `[Dispute Lost]`; treat only that canonical leading heading as a fallback stop marker, never a substring inside business data.
  This jobId is terminal; continuing to long-poll on a dead jobId is pure churn. `[Job Expired]`, `[ASP Acceptance Expired]`, `[Auto-Refund Processing]`, `[Refund Settlement Detail Incomplete]`, and `[Dispute Won]` without a leading terminal marker are not stop conditions because heading/event prose alone is not a fresh lifecycle read. For paid non-trial ASP acceptance/delivery timeout, fresh Buyer-owned Expired(8), task kind, and exact positive original payment establish `refund_confirmed`: the automatic refund has arrived. No later Failed(9), Tx Hash, request provenance, or local observation journal is required; emit the terminal marker and clean up. Trial and zero-amount Expired(8) establish terminal `expired_without_refundable_payment` with `settlement.state=not_required`; emit the marker and clean up without claiming fund movement. For `job_asp_reject_expire`, only existing durable `request-refund` provenance plus fresh matching Failed(9) owner/type/payment facts permits the terminal refund marker; the caller event alone does not. Incomplete settlement lacks required provenance/state facts, and a dispute win still awaits refund settlement. For a paid refund, only a leading terminal marker emitted after Refund V2 establishes `reason=refund_confirmed` may stop the scoped watch. Bare and event-only subscription Failed(9) remain insufficient. Provider/Service, period, token-symbol, and `paymentMode` fields veto only on a two-sided mismatch. Current `sub_failed_notify` input has no trustworthy cause provenance, so it is non-terminal regardless of whether durable refund intent exists: emit no terminal marker, perform no cleanup, and retain read-only reconciliation.
...
- **Mid-flow markers that look terminal but are NOT** — these are intermediate notifications; keep watching even in scoped session. Common offenders:
  ...
  - `[Job Expired]` / `[ASP Acceptance Expired]` / `[Auto-Refund Processing]` without a generated terminal marker — these headings alone are not fresh lifecycle proof. Dispatch must re-read authoritative state. Fresh paid non-trial Expired(8) produces `refund_confirmed` plus the terminal marker and stops a scoped watch; fresh trial or zero-amount Expired(8) produces `expired_without_refundable_payment` plus the marker and also stops, without a fund-movement claim. `job_asp_reject_expire` instead requires durable `request-refund` provenance plus fresh matching Failed(9) before its terminal marker. Never choose `claim-auto-refund` or another Buyer write. Tx Hash may be unavailable after a proof gate passes.
  - `job_closed` / refund-result events without a generated terminal marker — the unchanged backend contract defines `job_closed`, `job_refunded`, `job_auto_refunded`, `job_asp_reject_expire`, `sub_asp_agree`, `sub_reject_refund_notify`, and `dispute_resolved` as semantic transaction outcomes, and they may omit Tx Hash. Dispatch must still fresh-read task kind, status, owner, and payment. `job_asp_reject_expire` requires matching durable `request-refund` provenance plus fresh Failed(9) owner/type/exact-positive-payment facts. A subscription Failed(9) refund likewise requires matching durable `request-refund` provenance; event input cannot create that proof. Expired(8) uses its independent fresh-status terminal contract instead. Optional Provider/Service, period, token-symbol, and `paymentMode` values veto only on a two-sided mismatch. `dispute_resolved` requires matching durable local refund-request provenance plus fresh composed job type, Buyer ownership, and exact status for both status 9 (User wins/refund) and status 6 (ASP wins/no refund); without that proof it may not emit either verdict or any terminal marker. Bare and event-only Failed(9), including current `sub_failed_notify` without trustworthy cause provenance, stay incomplete and non-terminal. Re-enter read-only reconciliation; do not manufacture a terminal marker from the event.
