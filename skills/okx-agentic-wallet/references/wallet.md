...
1. **Check state.** Run `wallet status`. If `data.loggedIn` is `true` and this is not an explicit re-login, confirm the session to the user and continue with the rest of a compound request. Never render a subscription or device list from `wallet status`; those lists are produced only after a newly completed login or by an explicit user-initiated list command. If not logged in, or on an explicit re-login request, continue.
