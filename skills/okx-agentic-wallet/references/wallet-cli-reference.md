...
- `--phase poll` → persists the authenticated session, sends one best-effort device-registration heartbeat (`chainIndex=196`), then returns `accountId`, `accountName`, `loginType`, `isNew`, addresses, `totalValueUsd` (true `isNew` → new user; trigger the Policy Settings template — see [portal-actions.md](wallet-portal-actions.md)). When a non-empty User `agenticId` is resolved, it independently queries subscriptions and returns `postLoginSubscriptions: { activeSubscriptionCount }` only when the active count is positive. The field is omitted when `agenticId` is unavailable, no active subscription exists, or the subscription lookup fails or times out. Login does not query or update device routing; a heartbeat failure never turns a successful login into a failed login.
