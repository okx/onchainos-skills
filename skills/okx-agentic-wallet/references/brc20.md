...
4. Follow `selectionPlan.status`: present exact combinations in returned order and ask the user to choose when needed; for `NO_EXACT_MATCH`, run the paired balance and transferable-UTXO reads before deciding whether to offer the separate inscription flow; for `SEARCH_LIMIT_EXCEEDED`, report the incomplete result and current choices. Field and limit details are in the CLI reference.
...
| --- | --- | --- |
| BRC-20 holdings | ticker | Run the paired balance and transferable-UTXO queries |
