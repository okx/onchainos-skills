...
- **Not logged in** (`not logged in`): session expired or store missing. **MUST**: recover with the full `init` → `poll` orchestration, not a bare `wallet login` — run `wallet login --phase init`, then the `nextSteps.completeLogin` command it returns (`wallet login --phase poll --session-id <authSessionId>`); credentials are persisted only on the `poll` response, so `init`'s `ok:true` is not login-complete. See [wallet.md](wallet.md) Authentication.
