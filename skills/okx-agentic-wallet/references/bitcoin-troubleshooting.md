...
| --- | --- | --- |
| `44001` / `INSUFFICIENT_UTXO` | Service cannot select enough usable BTC UTXOs | Run `nextSteps.queryUnavailableUtxos`; unlocking is a separate confirmed intent. |
...
Service `error` and `data.message` are the user-facing explanation. Unknown failures end the current operation with the returned code and message; a new query establishes fresh facts.

## Failed Write Diagnostics

**MUST**: After a user-requested safe retry also fails, create a support diagnostic with `txHash` when available, `bitcoin` chain, service code/reason, asset, amount, wallet address, timestamp, and CLI version. **NEVER** expose raw codes in the user-facing response; use the returned service message instead.
