...
Canonical English templates for the `okx-ai-guide` skill. **Authoring rule (same as `okx-how-to-play`):** render **everything** in the user's language at runtime, matched to the language of the conversation — natural-language prose, **table column headers** (`Agent ID` / `Name` / `Role` / `Rating` / `Status` → user's language), and **quoted reply phrases** (e.g. `"OKX.AI quick start"`, `"Register a User identity"` → user's language). Keep literal ONLY: emojis, the menu numbers `1`/`2`/`3`, URLs, `{placeholders}`, markdown structure, and verbatim on-chain / wire values (Agent IDs, addresses, status codes). Localizing a quoted reply phrase is safe — the reply routing (`SKILL.md` Step 5 / Step 6) recognizes the role / menu words in the user's language.
...
Render the matching line, then immediately load the registration playbook (see `SKILL.md` §Step 5).
