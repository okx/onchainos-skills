---
name: okx-dex-social
description: "Use this skill for crypto social signals — news feeds, market-wide sentiment, and per-token vibe / KOL chatter. Covers: (1) news — latest feed, filter by coin symbol, full-text search, single-article detail, and the live source-platform list (blockbeats, odaily, theblock, etc.); (2) sentiment — top coins by mention activity (24h/72h/7d/30d) and per-coin sentiment with optional time-bucketed trend; (3) vibe — token-level hotness score with timeline + sample KOLs, and a TOP50 KOL leaderboard discussing a token (sortable by engagement / mentions / impressions). Use when the user asks 'latest crypto news', 'BTC news', 'news on ETH', 'search news for <keyword>', 'show news article <id>', '最新加密新闻', '搜索新闻', 'how bullish is BTC right now', 'sentiment for ETH', 'top coins by sentiment', '市场情绪', '情绪排行', 'who's talking about <token>', 'top KOLs for this token', 'token vibe score', '热度走势', 'KOL榜', 'first-mention KOL', or wants any social/news/sentiment/KOL signal NOT tied to a wallet trade. Also handles Market API payment/计费/x402/402, Basic/Premium tier/quota/额度 questions, and MARKET_API_*_OVER_QUOTA / confirming:true responses on social endpoints. NOTE: smart-money / 大户 trade activity → okx-dex-signal. Per-wallet PnL/holdings → okx-dex-market or okx-wallet-portfolio. Token metadata / holders / hot-token list → okx-dex-token. Token security / honeypot scan → okx-security."
license: MIT
metadata:
  author: okx
  version: "1.0.0"
