...
> `job_disputed` / `vote_committed` / `vote_revealed` **对 evaluator 不是动作触发点**。它们是 provider/buyer/tx-receipt 的事件，evaluator 收到仅记录即可。
...
**Step 2 — 向用户展示金额 + 等确认（⚠️ 强制步骤，不允许跳过）。**
...
**硬性规则**：未收到用户明确确认前，**绝不执行 Step 3 的 CLI**。`evaluator stake` 是上链操作，资金占用 7 天起——静默发起 = 严重违反用户授权。
...
- **不处理 unstake/increaseStake/claim**：那是完整 staking 生命周期，本场景只负责首次/补充质押 handoff。
...
| 首次 / 补充质押 OKB 成为仲裁者（来自身份 skill 跳转） | `onchainos agent evaluator stake --amount <OKB数量>` |
