...
```
默认金额 = 100 OKB（= 累计门槛规则 的累计门槛，也是首次质押场景的实际下限）
```
...
| 纯数字，使 `已有质押 + N < 100` | 告知"累计质押门槛 100 OKB（累计门槛规则），当前金额不够，请加大数额"，回到 Step 2 重新问 |
...
| 错误 `1001` | 累计质押门槛 100 OKB（累计门槛规则），当前金额 `<N>` 加上已有余额仍不够。回到 Step 2 请用户改数。 |
...
> ⚠️ 下表所有比例 / 人数 / 时长均来自 经济参数附录 参数默认值（`ParamsGovernance` 合约可调），待后端 `/staking/config` 接口上线后改由配置注入（见 §13）。
...
**部分赎回最低保留（部分赎回保留规则）**：部分赎回后剩余质押必须 ≥ **100 OKB**，否则只允许全额赎回。当前为写死值；`/staking/config` 上线后改为运行时从 `partialUnstakeMinRetainOkb` 拉取（见 §13）。在向用户确认金额前，若判断 `当前质押 - 本次 < 100 OKB 且 > 0` → 提醒："部分赎回后余额将低于最低保留 100 OKB，建议改为全额赎回。" 本地不阻塞，合约侧兜底。
...
- `request-unstake`：
  ...
  - 部分赎回保留规则：部分赎回后余额必须 ≥ **100 OKB**；低于此额只允许全额赎回（见 §12.1 "部分赎回最低保留"）
- `stake` / `increase-stake`：累计门槛规则 合约层按**累计**校验 `当前地址质押金额 + 本次金额 >= 100 OKB`——首次质押最低 100 OKB；后续追加无单次最低，但若账户余额不足 100（被 slash 后）需一次补齐到 100
...
Response.data:
  stakingConfig:
    minCumulativeStakeOkb:        100       # 累计门槛规则 累计门槛，§1.5 / §12.3
