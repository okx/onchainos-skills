...
- 🛑 **Calling `pending-decisions-v2 resolve` / `pick` / `cancel` / `list`** — these are **user-session-only** (queue file lives in the user-session's home; sub has no access). Real incident: sub received `{source:"system", event:"user_decision_job_submitted", data:"approve"}` and called `resolve --user-reply "approve"` → wasted a turn, then fell back to next-action. Correct: when you see `event:"user_decision_*"`, call `next-action --jobStatus <event-verbatim> --data "<message.data>"`. The user-session already resolved the queue before sending you this envelope; your job is to route the reply via next-action, NOT to re-resolve.
