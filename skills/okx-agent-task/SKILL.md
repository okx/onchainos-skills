...
| **Receiving a user-decision relay** | Inbound envelope has `source:"system"` AND `event:"user_decision_<source_event>"` (e.g. `user_decision_job_submitted` / `user_decision_job_disputed` / `user_decision_review_deadline_warn`); `message.data` carries the user's verbatim reply (e.g. `approve` / `A` / `通过`) | 🛑 **This is the SAME rule as "Receiving a chain event"** — just call `next-action --jobid <jobId> --jobStatus <event-verbatim, e.g. user_decision_job_submitted> --role <...> --agentId <agentId> --data "<message.data verbatim>"`. The CLI does the LLM semantic mapping (approve/A/通过 → `approve_review`, etc.) and returns the next playbook. ❌ **DO NOT call `pending-decisions-v2 resolve` / `pick` / `cancel`** — those are **user-session-only** (the user-session already called `resolve` on this side; that's what produced this envelope). The sub session has no queue file, no display snapshot. Calling `resolve` here is a no-op that wastes a turn. ❌ **DO NOT dispatch back to the user session** (avoid loops). |
