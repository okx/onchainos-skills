...
**正确流程**：收到首条 a2a-agent-chat → Step 1 看 `sender.role=1` 反推自己是 provider → Step 2 读 `provider.md` 1. 触发识别 → Step 3 调 `common context` 加载任务详情 → 按 provider.md 3. 协商阶段三项确认（价格 / 币种 USDT 或 USDG / 验收标准）→ 等用户拍板 → `apply` 上链 → 等 `job_accepted` 通知 → `deliver`。
...
详见上方 **Session 通信契约 4. sub session agent 状态机 - 接收链事件**。要点：
...
- **P2P 消息发送规则**统一走 Session 通信契约 5.4 的 `session_status` → `xmtp_send` 两步法，不要把正文当 agent 文字输出
