...
| 3 | user → sub | user-session calls `onchainos agent pending-decisions-v2 resolve --user-reply "<verbatim>"`; CLI returns a relay playbook telling user-session to call `xmtp_dispatch_session(sessionKey=<sub_key>, content=...)` exactly once | Default: `[USER_DECISION_RELAY] decision: <user verbatim>`. Intent-tag scenes (sub provided `--llm-content` with intent emission instructions): `[USER_DECISION_RELAY][intent:CODE] user said: <verbatim>` (CLI auto-detects the `[intent:` prefix and concatenates without the `decision: ` filler) | User-session agent (via `pending-decisions-v2 resolve`) | Sub agent (parses keywords / intent tag and calls `next-action --event <pseudo_event> --jobStatus <pseudo_event>`) | **Exactly once** after the user replies to a `[USER_DECISION_REQUEST]` |
