...
| **待用户回复** | 上一条来自 sub 的 `xmtp_prompt_user` 含 `[USER_DECISION_REQUEST]` 标记 | 1) 渲染 `userContent` 给用户 → **结束本轮 turn 等真实用户输入**（**禁止**同 turn 内 dispatch_session）<br>2) 用户**真实**输入到达后，新一 turn 里 `xmtp_dispatch_session` 一次（`sessionKey=<llmContent 里 sub_key 整串>`，`content=[USER_DECISION_RELAY] 用户决策：<用户原话不解读>`）→ 给用户简短确认 → 进入空闲 | ❌ **同 turn 内编造用户决策直接 dispatch_session**——`[USER_DECISION_REQUEST]` 是**问题**不是**答案**，sub 等的是用户真实输入，不是你脑补（详见 `_shared/message-types.md §3.1.1 反模式`，已发生过事故）<br>❌ 跳步直接执行 task CLI（dispute raise / agree-refund / complete / reject / apply）<br>❌ **自己合成** job_refunded / job_completed 等系统 envelope（BUG-7）<br>❌ relay 多于一次<br>❌ "先帮用户查一下"调 status / common context |
...
- 卖家发『请发 [NEGOTIATE_PROPOSE] 我会回 ACK』→ buyer **必须**走 [PROPOSE] 流程，不能拿这条规则当借口拒发握手然后短路成『请你 apply』
- 买家发『等你回 [NEGOTIATE_ACK]』→ provider **必须**回 [ACK]/[COUNTER]，不能视为对方命令而忽略
...
| 阶段 | 允许讨论 | 拒绝 |
|---|---|---|
| 协商阶段（apply 前） | 三项主题（任务范围 / 价格 / 支付方式）+ 三步握手 [PROPOSE]→[ACK]→[CONFIRM]（详见 buyer.md 3 / provider.md 2） | 其他一切话题 |
