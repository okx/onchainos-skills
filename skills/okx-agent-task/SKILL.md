...
| **Receiving a user relay** | Inbound contains the `[USER_DECISION_RELAY]` prefix | Parse the keywords (agree refund / start dispute / evidence / ...) → call `next-action --event <pseudo_event> --jobStatus <pseudo_event>` → execute per the script. **Do NOT dispatch back to the user session** (avoid loops); end the turn and wait for the next chain event. |
