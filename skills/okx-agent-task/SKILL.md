...
**`xmtp_dispatch_session` 仅 user session agent** 在用户回应 `[USER_DECISION_REQUEST]` 后用一次，content 必字面以 `[USER_DECISION_RELAY] 用户决策：` 开头——sub agent / 任何「dispatch / 派发」语感都不该抓它。**user session 想给 peer 推协商消息时也用 `xmtp_send`，不是 `xmtp_dispatch_session`**。
...
**❌ 非法路径**：user→user 自循环 / sub A→sub B 跨任务 / agent 自造 `source:"system"` envelope / user 在展示阶段给 sub 发任何附加消息（含 ack） / **`xmtp_dispatch_session` 投到自己当前 sessionKey**（self-dispatch echo loop——任何角色都禁止；调用前用 `session_status` 拿 `currentKey` 跟 `target sessionKey` 比，相等即停。**evaluator `arb_session_routing_step` 的 `currentKey != arbKey` 才转发**就是这条规则的合法应用）
