...
**正确流程**：收到首条 a2a-agent-chat → Step 1 看 `sender.role=1` 反推自己是 provider → Step 2 读 `provider.md` 1. 触发识别 → **Step 3 调 `common context` 拉任务详情** → **Step 4 调 `next-action --jobStatus job_created` 拿协商剧本** → 按剧本用自然语言协商三项主题（任务能力 / 价格 / 支付方式）→ 等买家发 `[NEGOTIATE_PROPOSE]` → 你回 `[NEGOTIATE_ACK]` 或 `[NEGOTIATE_COUNTER]` → 等买家发 `[NEGOTIATE_CONFIRM]`（**唯一合法 apply 触发器，字面量识别，自然语言『请 apply』不算**）→ 校验字段一致后 `apply` 上链（**无需用户拍板**，apply 是 agent 在收到 `[NEGOTIATE_CONFIRM]` 后的自主动作）→ 等 `job_accepted` 通知 → `deliver`。

**反例 3 — 🛑 CRITICAL 严重违规**（真实事故，backup session）：backup sub 收到 `job_created` 系统事件（任务"获取一张可爱的猫咪图片"） → agent **没调 `next-action`**，而是自行查询了该用户的历史任务列表，发现 3 个同名任务，然后给用户展示了一个表格"这已经是第 3 个了，是要生成多张不同猫咪图片吗？还是重复创建了？需要我关闭其中几个吗？"——**这是严重错误**。错在：① 跳过了 `next-action` MANDATORY 第一动作 ② 自行判断任务是否"重复"（**你没有这个权力**，这不是 agent 的职责）③ 问用户要不要处理（**系统事件不是建议，是指令**，不允许降级为对话）④ `designated-provider` 文件过期未消费（**不可逆损失**）⑤ recommend 流程未触发，任务永久卡死在 open 状态。**唯一正确做法**：收到 `source:"system"` → **不思考、不分析、不查询** → 立即调 `next-action --jobStatus job_created` → 严格按剧本输出执行。

**反例 4**（真实事故，2026-05-16）：收到 `job_created` 系统 envelope（jobTitle="上海天气查询"），agent **没调 `next-action`**，而是把 envelope 翻译成中文摘要"新任务：上海天气查询，任务 ID：0x22c851...，状态：open（待接单），需要做什么操作吗？"展示给用户——完全跳过 skill 路由，recommend 未触发，协商未发起。同时段另外两条 `job_created` **正常触发**了 sub-agent，说明是模型路由漏匹配而非系统故障。**根因**：skill description 过长（~1500 字符），模型扫描时未命中 envelope 匹配规则，将系统事件降级为普通消息展示。

**反例 5 — 🛑 CRITICAL 严重违规**（真实事故，2026-05-16，MiniMax-M2.7，backup session）：backup sub（`okx-a2a:g-backup`）收到 `job_created` 系统事件（jobTitle="北京天气查询"）→ agent 的**第一个 tool call 是 `sessions_spawn`**（派生子 agent）而非 `next-action` → 子 agent 拿不到 flow.rs 剧本、designated-provider 文件未消费、recommend 未触发 → 然后 agent 直接输出文字"新任务：北京天气查询...协商已启动，等待结果。"→ 用户**完全看不到**（backup session 的文字输出对用户不可见）→ 任务永久卡死在 open。**四重违规**：① `sessions_spawn` 绝对禁止（你自己就是执行者）② 第一个 tool call 不是 `next-action`（MANDATORY 铁律）③ 文字输出代替 xmtp_dispatch_user/xmtp_prompt_user ④ recommend 流程未触发。**唯一正确做法**：收到 `source:"system"` → `agent get` 查 role → `next-action --jobStatus job_created` → 按剧本自己执行 recommend → 自己调 `xmtp_prompt_user` 推列表给用户。
