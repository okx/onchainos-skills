...
>    - ⚠️ **本命令仅用于 Pre-flight 自检"我有没有这个角色的 agent"**——**严禁**用它判断 envelope 顶层 `agentId` 是否属于你。`my-agents` 只列**当前活跃 account**，跨 account 的 agent（如同钱包下别的 account 名下的 evaluator）会被静默过滤掉。envelope 顶层 `agentId` 一律走 `onchainos agent profile <id>` / `agent get --agent-ids <id>` 直查 role（见 `## Activation` Step 1）。
...
### a2a-agent-chat 入口三步（**仅 a2a-agent-chat**；system envelope 走上方 MANDATORY 段落，不进本节）

#### Step 1 — 识别自己的角色

- 看 `sender.role` 反推——`sender.role=1` 对方是 buyer → 我是 **provider**；`sender.role=2` 对方是 provider → 我是 **buyer**
...
角色识别后立即读 [`buyer.md`](./buyer.md) / [`provider.md`](./provider.md) 之一（evaluator 角色不收 a2a-agent-chat），按 1. 触发识别 + 后续 scene 走。**禁止**只读 SKILL.md 就回复——SKILL.md 只定义跨角色协议，role-specific scene 都在 role 文件里。
...
返回【当前状态】+【双方信息】+【可执行操作】，给 agent 补齐协商参数 / 支付方式 / 协商进展等本 turn 决策必需的信息。**只读 API，可多次调用，不会改 status。** ⚠️ system envelope 入口下严禁在 `next-action` 之前调用本命令。
...
2. 再调 `xmtp_send`：
   - `sessionKey` = 第 1 步那串
   - `content` = 纯自然语言（插件自动包成 a2a-agent-chat envelope；**不要**自己写 `jobId:`/`类型:`/`----` 这种 text-header，**不要**包 markdown 代码块）
   - `payload` = 协议版本握手 JSON，值见 next-action 剧本输出顶部的 `[Protocol version]` 行（语义详见 `_shared/message-types.md` 的 `payload.taskMinVersion` 字段）
