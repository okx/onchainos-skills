...
| 错误码 / 信息 | 触发原因 | 处理 |
|---|---|---|
| `code=1001` + `msg` 含 `text or images required` | `dispute upload` 没传 `--text` 或 `--image`（参数校验失败） | text / image 至少一项；text 长度上限 16KB（CLI 已加 pre-check） |
...
| `dispute info` 找不到 disputeId | disputeId 拼写错误或还没创建 | `disputeId = keccak256(jobId, roundNumber)`；envelope 自带，**不要**用 `d-<jobId>-r1` 占位符 |
