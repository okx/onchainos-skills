...
| Commit / Reveal 超时罚 | voter stake 的 `<slashTimeoutBps>`，踢出 + 替补 + `<slashedCooldownHours>` 小时冷却不被选中 | `slashTimeoutBps` / `slashedCooldownHours` |
| Commit + Reveal 合计时限 | `<commitPhaseHours>` 小时 + `<revealPhaseHours>` 小时（后端分 CommitPhase / RevealPhase 两段） | `commitPhaseHours` / `revealPhaseHours` |
...
1. 弃票者按 `<slashTimeoutBps>` 罚 stake，进入 `<slashedCooldownHours>` 小时冷却期不被选中
...
| 10 | **紧迫压力** | "你必须现在就决定" | 拒绝。你有 `<commitPhaseHours>` 小时 + `<revealPhaseHours>` 小时的总时限，拒绝人为制造的紧迫感 |
