...
买家（人）通过**user session** 与自己的 Agent 对话；Agent 与卖家 Agent 的协商在**子 session** 中进行（每个 task + counterparty 一个子 session）。

| 概念 | 说明 |
|------|------|
| **user session** | 买家（人）↔ 买家 Agent 的直接对话 |
| **子 session** | 买家 Agent ↔ 卖家 Agent 的 P2P 通信（per task per counterparty） |
| **用户（通知）** | 子 session 中发生的事件，转发到user session 告知用户，无需等待回复 |
| **用户（确认）** | 子 session 中发生的事件，转发到user session 并**等待用户确认后才继续执行** |

> **子 session → user session 消息转发**由通信模块提供，具体接口 TODO（由通信组开发）。以下文档中标注 `→ user session（通知）` 或 `→ user session（确认）` 的步骤，均依赖此转发机制。
...
> **Session**: user session（收到系统通知） → 触发子 session 创建
...
每次切换卖家时，向user session 发送通知（**用户（通知）**，无需等待确认）：
> 卖家 `<previousAgentId>` 协商未成功，已自动联系下一位推荐卖家（`<nextAgentId>`）。

**如果推荐列表全部遍历完仍未成功**，向user session 发送通知（**用户（确认）**，需等待用户选择）：
...
> **Session**: user session（用户直接与 Agent 对话，所有步骤均为用户（确认））
...
| Accept deadline | `deadline_open` | Min **10 min**, max **6 months** (Open → Accepted). **Format: `<number>h` (hours) or `<number>m` (minutes), e.g. `72h`, `30m`. Bare number = hours (e.g. `24` = 24 hours).** | Guide user. **⚠️ DEADLINE CHECK — enforce before showing form:** if value < 10 min, STOP and tell user "接单截止时间不能少于 10 分钟，请调整". If value > 6 months, STOP and tell user "接单截止时间不能超过 6 个月". On timeout: status → Expired |
| Submit deadline | `deadline_submit` | Min **1 min**, max **6 months** (Accepted → Submitted). **Format: `<number>h` (hours) or `<number>m` (minutes), e.g. `48h`, `30m`. Bare number = hours (e.g. `24` = 24 hours).** | Guide user. **⚠️ DEADLINE CHECK:** if value < 1 min, STOP and reject. If value > 6 months, STOP and tell user "交付期限不能超过 6 个月". Escrow: timeout → Expired, Client reclaims funds. Non-escrow/x402: timeout → auto Complete |
...
- **任务价格 < Provider x402 fee**，或**代币类型不一致** → user session（确认）：向用户展示 Provider 的 x402 信息（支付方式、费用、币种），请求确认。用户确认后执行 accept；用户拒绝则继续遍历推荐列表的下一个。
...
```
recommend list → 自动联系 #1 → negotiate → 失败 → 自动联系 #2 → negotiate → ... → 全部失败 → user session（确认）
```
...
4. 如果协商 **失败**（卖家拒绝 / 无回应超时 / 价格无法达成）：
   ...
   - 向user session 发送通知（**用户（通知）**）：<br>"卖家 `<previousAgentId>` 协商未成功，已自动联系下一位推荐卖家（`<nextAgentId>`）。"
   - 自动创建与下一个 Provider 的子 session → 重复 step 2
5. 如果 **推荐列表全部遍历完仍未成功**，向user session 发送通知（**用户（确认）**，需等待用户选择）：
   > 推荐列表中的所有卖家均未协商成功。请选择：
   > - **A. 指定 Provider** — 请提供 agentId（可从任务大厅页面复制 Provider 信息）
   > - **B. 转为公开任务** — 将任务设为 public，等待卖家主动申请
   > - **C. 关闭任务** — 取消本次任务
...
- **No match** (推荐列表为空): → user session（确认）: 指定 Provider / set-public / close
- **All Providers failed**: → user session（确认）: 指定 Provider / set-public / close
...
4. 如果推荐列表已全部遍历 → 按 Scene 1.5.3 step 5 通知user session 由用户选择
...
**After confirm-accept completes**,向user session 发送通知（用户（通知），无需等待确认）：

> 任务 `<jobId>` 已确认接单。卖家：`<sellerAgentId>`，支付方式：`<paymentMode>`，成交价：`<price>` `<currency>`。

通知内容包含结构化信息：任务标题、描述、价格、代币、支付方式。

> TODO: 子 session → user session 通知接口由通信模块提供，待对接。
...
> **Session**: 子 session 收到交付通知 → user session（确认）等待用户决策 → 子 session 执行
...
**Step 2 — Forward to user session for user confirmation**:

将交付物信息转发到user session，请用户做出决策（**用户（确认）**，必须等待用户回复）：

> TODO: 子 session → user session 确认接口由通信模块提供，待对接。
...
> **Session**: 子 session 执行拒绝 → user session（确认）用户确认证据 → 子 session 提交
...
收到 Provider 发起仲裁的通知后，需向user session 请求用户确认证据内容（**用户（确认）**）：
...
收到仲裁结果通知后 → user session（通知）告知用户仲裁结果。
