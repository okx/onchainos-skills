...
>
> 0. **Skill prefetch** (source: self via `xmtp_dispatch_session`): content starts with `[SKILL_PREFETCH]` → **load `okx-agent-task` SKILL.md + `buyer.md` into context**. Skip the prefetch message (no reply, no CLI, no sends) and process any other peer messages in the same turn via #1–#6 as normal; end the turn only if no other messages remain.
>    🔴 Real incident: prefetch + ASP quote arrived together → agent applied "no action" to the whole turn, skipped `negotiate_reply`, task stuck.
