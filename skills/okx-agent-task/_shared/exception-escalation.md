...
- 对方下一条 inbound envelope 里**还在重复同一个错误诉求**（如 escrow 路径还在向你索要 `paymentId`、对协商已确认的字段反复改口）
...
**Role-specific 例外（evaluator）**：`evaluator commit` / `evaluator reveal` / `evaluator claim` 因 commit / reveal 窗口关闭直接罚 0.3% stake，allow sub 内部最多重试 3 次——这是仲裁经济模型逼出来的硬约束，详见 `references/evaluator-decision-rubric.md` 6。其他 evaluator 命令（`stake` / `unstake-*` / `info` / `download` 等）仍走 2 标准流程。Buyer / provider 没有此类例外。
