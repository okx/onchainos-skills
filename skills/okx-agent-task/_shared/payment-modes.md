...
| 模式 | 符号 | 适用场景 | 资金流向 |
|---|---|---|---|
| **Escrow（托管）** | `escrow` / `1` | 默认、推荐；双方互不信任的新关系 | 买家 confirm-accept 时资金锁进合约；complete 时合约自动放给卖家 |
| **Non-Escrow（直接付款）** | `non_escrow` / `direct` / `2` | 买卖双方有信任基础；或小额任务 | 协商后卖家 create_payment_charge 生成账单 → 买家 a2a_pay 支付 → direct/accept → direct/complete 上链；卖家后续提交交付物 |
| **x402** | `x402` / `3` | 按次付费的 API / 服务调用 | 买家 confirm-accept 时调 provider endpoint，走 HTTP 402 签名重放完成支付 |
...
|---|---|---|---|
| escrow | 卖家 apply 上链（provider_applied） | `onchainos agent confirm-accept <jobId> --provider <p> --payment-mode escrow` | 资金托管到合约；pre-accept 双签流程 |
...
| 模式 | 触发时机 | 买家 CLI | 资金动作 |
|---|---|---|---|
| escrow | submitted → completed（验收交付物后） | `onchainos agent complete <jobId>` | 合约 pre-complete 双签 → 自动释放托管给卖家 |
...
| 模式 | 触发时机 | 付款单内容 |
|---|---|---|
| escrow | 协商达成后卖家 apply 上链，收到 `provider_applied` | 金额、币种、托管合约地址、paymentMode=escrow |
