...
### pending-decisions-v2 request / resolve-prompt / pick / cancel / list
...
Pending-decisions queue. `handle_request` writes each entry as `Status::Queued` keyed by `sub_key`; same sub_key overwrites in place. Routing on user reply uses the pre-filled `resolve-prompt` command embedded in each block's llmContent. File `$ONCHAINOS_HOME/task/pending-decisions-new.json` (or `~/.onchainos/task/...` if env unset), with companion `last-display.json` snapshot for stable pick indexing. Concurrent-safe via `fs2` file lock + `tempfile` atomic rename; cross-platform. Authoritative rules: `SKILL.md Session Communication Contract §5. pending-decisions-v2`.
...
| `pick` | user-session agent | Use when the user wants to revisit / expand a card from the decision list (`pending-decisions-v2 list` then user picks `1` / `第 1 个` / `选 2`). Re-displays the selected card's full content via `playbook_render`; no status mutation, no queue modification. Stale-selection detected via `last-display.json` snapshot timestamps. | `--index` (required, 1-based integer matching the displayed list). Behavior: locate the entry by snapshot index, then emit `playbook_render(entry)`. Pick is purely "show me this card again" — the actual decision routing on user reply still uses the block's pre-filled `resolve-prompt`. |
