...
> Self-contained reference for the user's sub-sessions (task sub and backup sub). The user-session's free-text routing is in `router.md`; its selected operation rules are in `playbook.md`. They are not covered here.
...
System events (`message.source == "system"`) → follow `../core.md` `## Activation` #1. Supplements beyond what Activation covers:
...
| `sub_asp_agree` / `sub_reject_refund_notify` | Run `next-action`, then apply the settlement and terminal rules in [`refund.md`](refund.md). Render only the returned result; the event itself neither proves settlement nor authorizes a Buyer write. **end turn**. |
| `sub_complete_notify` | Route the structured result through [`../../shared/task-action-routing.md`](../../shared/task-action-routing.md). |
| `sub_failed_notify` | Run `next-action` and render its returned read-only result. This event does not independently establish refund or charge-failure cause; follow [`refund.md`](refund.md) and emit no terminal effects unless the structured result provides them. **end turn**. |
| `sub_close_notify` | Fresh subscription detail must prove Buyer ownership and Closed(7). Show the returned close result, preserving `aspRejectReason` verbatim when present. If the result indicates refund reconciliation, follow [`refund.md`](refund.md); Closed alone is not refund settlement. |
...
Use [`refund.md`](refund.md) as the only settlement and
...
> **Tool readiness is hinted at `task-create-prepare` time and re-checked on every real signal.** Subscription creation never silently installs a plugin or grants trading authority. When `next-action` returns `active_subscription_signal`, follow [`execution-policy.md`](execution-policy.md). Historical contexts are resumed through the same Guide-driven direct lifecycle. The signal flow owns model classification, visible setup, authorization, and tool execution.
...
to `playbook.md` §Pause auto copy-trade. Do not duplicate or execute
