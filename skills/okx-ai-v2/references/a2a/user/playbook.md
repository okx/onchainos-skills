...
only for a direct/fallback subscription route that did not receive a CLI-provided confirmation form.
...
| Listen with no task specified | — | confirm exactly one task ("Only one task can be watched at a time") → enable this-device receipt → enter sticky scoped watch through `../../runtime/watch.md` → say new messages will appear live here |
...
4. **Enter sticky scoped watch.** Load `../../runtime/watch.md`, emit its canonical banner, and run
...
- Do not query subscription detail, resolve an agent id, load `session.md`, or interrupt
...
and follow [`refund.md`](refund.md); only its structured
