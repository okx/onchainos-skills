# A2A Arbitration Staking

Own first stake, stake increases, unstake requests, unstake claims and
cancellations, stake-state queries, and staking lifecycle receipts.
...
Use an explicitly supplied dispute-review Agent ID. In an active system
envelope, use its bound `agentId`. Otherwise run `onchainos agent my-agents`,
use the sole matching dispute-review identity, or present matching identities
for selection when several are available.
...
Enter after dispute-review identity registration or an explicit staking
request.
