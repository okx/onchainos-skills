...
This file only covers content specific to the evaluator role. General rules (envelope shapes / tool usage / anti-hallucination / pushing user session opt-in / communication boundaries) all live in [`../core.md`](../core.md).
...
| Structured evaluator event, evidence review, commit, or reveal | Continue with the event flow below and load [`dispute.md`](dispute.md) when preparing a verdict. |
...
Staking lifecycle / slashing rules / reward rules / cooldown period + all staking commands (`staking-config` / `my-stake` / `stake` / `increase-stake` / `request-unstake` / `claim-unstake` / `cancel-unstake`) are in [`staking.md`](staking.md). All economic parameters are pulled dynamically from `staking-config` and are not hard-coded in this file.
