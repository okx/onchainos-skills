...
> Loaded from `watch.md` when the user explicitly asks to see decision_request items they have **not yet replied to**. This is a user-initiated one-shot query, separate from the live watch loop — it does NOT long-poll and does NOT re-enter watch.
...
> 🛑 Run exactly as written — no `| grep` / `| jq` / redirects (same JSON-integrity rule as watch; see `watch.md` §Anti-patterns).
...
2. For each item, paste its `userContent` as a markdown blockquote (same copy-not-rewrite rule as `watch.md` §Dispatch — no wrapper sentences, no summarization, no cross-item merging).
...
Once the item is identified, claim it and execute its `llmContent` using the same flow as `watch.md` §Handling the user reply (claim via `okx-a2a user check --todo-ids <id> --json`, then run whatever commands `llmContent` specifies).
