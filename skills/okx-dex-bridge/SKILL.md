...
| 4 | `onchainos cross-chain approve --chain ... --token ... --wallet ... --bridge-id ... (--amount <raw> \| --readable-amount <human>) [--check-allowance]` | Build ERC-20 approve tx for a bridge router (manual use). Pass **exactly one** of:<br>• `--amount <raw>` — smallest token unit, e.g. `500000` for 0.5 USDC at 6 decimals. Use `--amount 0` to revoke (USDT pattern).<br>• `--readable-amount <human>` — human-readable decimal, e.g. `0.5` for 0.5 USDC. CLI fetches token decimals via token-info and converts to raw automatically. To revoke use `--amount 0` instead (human "0" is meaningless).<br>The two flags are mutually exclusive — clap rejects passing both. |
