...
| 4 | `onchainos cross-chain approve --chain ... --token ... --wallet ... --bridge-id ... --amount <raw> [--check-allowance]` | Build ERC-20 approve tx for a bridge router (manual use). `--amount` is **raw integer in smallest token unit** (e.g. `500000` for 0.5 USDC at 6 decimals — different from `quote`/`swap`/`execute`'s `--readable-amount` which auto-converts via decimals). Pass `--amount 0` to revoke (USDT pattern). |
