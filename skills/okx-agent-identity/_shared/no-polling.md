...
4. **No speculative side-queries.** Do not run `wallet status` / `agent get` / `agent search` "just to be safe" before the user's actual command. Pre-flight checks in `_shared/preflight.md` happen once per session; after that, trust the state. **Concrete examples**:
   - After `agent get --agent-ids <id>` returns the single-agent detail, do **NOT** chain `agent service-list --agent-id <id>` — the `services` array is already in the response (`list[0].services`). Do **NOT** chain `agent feedback-list --agent-id <id>` — the reputation aggregate `{ score, count }` is already in the response; pull the full review list **only if** the user says yes to the numbered-options prompt in `display-formats.md §Post-detail prompt`.
