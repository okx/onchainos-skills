...
4. **Phase boundary is strict.** Identity-phase capture does NOT reach into service-phase fields. "provider 叫 Alice，收 10 USDT" → capture `name=Alice`, discard `fee=10`. When Phase 2 starts, MAY quote the earlier mention as a suggested default (not auto-fill).
5. **All fields captured → still render confirmation card.** Even if every required field was covered in one shot, the confirmation card is mandatory. Wait for explicit `执行` / `execute` / `yes` before calling the CLI.
...
- **A — partial, requester:** "注册一个买家叫 Alice" → captures `role=requester`, `name=Alice`. Skip Q1 → Q2 (picture) → confirmation.
- **B — full, requester:** "注册一个买家，名字 Alice，不要头像" → All Q's skipped → confirmation card directly.
- **C — ambiguous split:** "provider 叫 Alice 做 DeFi 分析师" → captures `role=provider` only; name + description left for normal Q&A.
- **D — cross-phase leakage (strict rejection):** "provider 叫 Alice，做 DeFi 分析，收 10 USDT" → Phase-1 capture: `name=Alice`, `description=做 DeFi 分析`. **Fee=10 is discarded.** Phase 2 starts fresh with its own Q1.
