...
| `onchainos agent submit-approval` | Submit agent for listing review (skill-internal — triggered automatically after `activate` returns `approvalStatus: 1`; never invoked directly by the user) | `--agent-id` | — |
...
| `agent activate --agent-id <id>` — `success: true` only | Proceed to `§Step 6`. For all other outcomes (`approvalStatus: 1/2/5`, `code: 81602`) the activate row in `§Suggest Next Steps` stops the turn; do NOT proceed to `§Step 6`. |
...
| `agent activate` | The activate call returns one of five outcomes; handle each before deciding whether to proceed to `§Step 5` → `§Step 6`. **No `agent <cmd>` CLI literal in user-visible text** (Red lines 1/2).<br><br>**Outcome A — `success: true` (listed immediately):** Render declarative success line (no question mark, no offer soliciting a reply — Step 5 → Step 6 runs immediately after): CN "上架成功 — 你的 agent 现在已经能被市场搜到。" / EN "Published — your agent is now discoverable on the marketplace." Then proceed to `§Step 5` → `§Step 6`.<br><br>**Outcome B — `success: false, approvalStatus: 1` (review not yet submitted):** Call `onchainos agent submit-approval --agent-id <id>` immediately (skill-internal, no user prompt). Handle the submit-approval result per `references/cli-reference.md §11` and `troubleshooting.md §2`. **Stop after rendering; do NOT proceed to `§Step 5` / `§Step 6`.**<br><br>**Outcome C — `success: false, approvalStatus: 2` (already under review):** Render the review-pending message per `troubleshooting.md §2`. **Stop; no `§Step 5` / `§Step 6`.**<br><br>**Outcome D — `success: false, approvalStatus: 5` (review rejected):** Render the rejection card (with `rejectReason` when non-empty) per `troubleshooting.md §2`. **Stop; no `§Step 5` / `§Step 6`.**<br><br>**Error — `code: "81602"` (agent blacklisted):** Render the blacklist error per `troubleshooting.md §2`. **Stop; no `§Step 5` / `§Step 6`.** |
