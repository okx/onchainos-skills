...
- [ ] Step 1: 路由 — 按上表选 reference ⛔ BLOCKING
- [ ] Step 2: 加载 reference + invariants.md，按 reference 步骤执行 ⚠️ REQUIRED
- [ ] Step 3: 执行 CLI → 渲染输出（读操作: 按 reference 模板; 写操作: card → confirm → CLI → 模板）→ 输出前核查
- [ ] Step 4: 成功 → §Step 5/6；失败 → `references/errors.md`
...
## Pre-Delivery Checklist (每次用户可见输出前)

- [ ] 无 `onchainos` 字面量 / skill 名 / A2MCP·A2A 原始枚举
- [ ] `*Label` 字段已翻译至会话语言
- [ ] 写操作（create/update）已展示 card 并等待用户确认
- [ ] 成功输出来自 reference 模板，非自行总结 JSON
- [ ] `#<id>` 来自 CLI 输出（invariants.md §id ladder），非推断或复用 pre-check id
