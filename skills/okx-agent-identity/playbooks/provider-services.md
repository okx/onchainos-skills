...
So a one-shot like "TVL 查询, API, 10 USDT, https://api.x/mcp, queries TVL" registers name/type/fee/endpoint from the batch, then **splits out** one focused step for the service description (because "queries TVL" is not the required 3-part structure). The simple fields never block; only the complex one gets the extra turn it actually needs.
...
- Then present the full draft for **explicit approval** ("用这个?" / edit). The user's edits win.
