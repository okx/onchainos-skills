...
Never leave the row blank, render a bare `—`, fabricate placeholder copy ("无描述" / "用户未填写描述" / "TBD"), or omit the row. Diff cards: when the current value is empty, the `当前值` / `Current` column reads `未填` / `(not set)`; when the new value is empty, the `新值` / `New` column reads the same. **An update that sets description to empty is treated as a real change** — the row is bolded in the new-value column just like any other diff.
