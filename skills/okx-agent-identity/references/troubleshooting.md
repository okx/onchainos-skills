...
| `creator agent not owned by caller` | "评价发起人必须是你当前钱包名下的 agent — 我重新走一遍发起人选择。" / "The reviewer must be an agent owned by your current wallet — let me redo the reviewer-selection step." (no `--creator-id` flag in user text — Red line 2; use `ux-lexicon.md §Field` mapping `creator-id` → 发起人 / reviewer) | Return to `feedback-guide.md §Step 2` and re-run ladder 2 from the top — the next user-visible message is whichever of the **3 branches** ladder 2 lands in: **0 agents** under current wallet → STOP and offer registration (do NOT promise to auto-pick); **1 agent** → silently use it and mention the choice in the next confirmation; **multiple agents** → ask the user with the numbered-options prompt and wait — `Do not auto-pick`. ⛔ The error-line wording above MUST NOT commit to "pick one and retry" because for the 0-agent / multi-agent branches that promise is wrong. |
