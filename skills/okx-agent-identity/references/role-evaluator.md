...
|---|---|---|---|
| Q1 | `你要注册的验证者身份叫什么名字？` + 4 segments | `What's the name of this evaluator?` + 4 segments | non-empty, ≤ 64 chars |
| Q2 | `用一句话描述你的仲裁领域或专长（可选，回车 / "跳过" 即不填）。` + 4 segments | `Describe your arbitration domain or expertise in a sentence (optional — press enter or reply "skip" to leave blank).` + 4 segments | optional; if supplied then ≤ 500 chars |
...
- If **both** source 1 and source 2 miss — i.e. CLI returned `txHash` only **AND** the post-create `agentList` segment is absent (WS + HTTP both failed, per `cli-reference.md §1`) **OR** the diff yielded no new candidate under the current wallet — → **omit the `#<id> ` substring entirely** — do NOT render `#`, `#<id>`, `# ?`, do NOT invent a number, and do NOT borrow an id from the pre-check list. The second line is unaffected. Fallback first lines:
  - Chinese: `验证者身份注册完成。`
  - English: `Evaluator identity registered.`
...
| "帮我直接质押再注册" | Correct them: "得先注册再质押。这边先建好验证者身份，我接着帮你走质押那一步。" (Skill internally hands off to the staking flow afterwards — do **not** name the skill path in user-visible text, Red line 1.) |
| "不想质押" | Offer: "验证者身份可以先注册放着，但没质押不会被派单。要不你考虑改注册买家或卖家身份？或者就保留这个验证者身份，想好了再质押。" (Chinese localization mandatory — do not leak raw `evaluator` / `requester` / `provider`, Red line 4.) |
