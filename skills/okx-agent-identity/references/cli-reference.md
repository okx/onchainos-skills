...
**Known limitation — DoH-proxy environments**: when the wallet HTTP client internally rewrites outgoing requests to a DoH proxy URL, the WS connection cannot follow that rewrite (the proxy substitution lives inside `WalletApiClient` and is not surfaced to identity callers). HTTP still works in those environments, but the WS connect lands on the raw identity host and the command falls through the soft-failure path: `agent` absent, broadcast and `agentList` unaffected. The push capability degrades silently in DoH-proxy mode by design.
