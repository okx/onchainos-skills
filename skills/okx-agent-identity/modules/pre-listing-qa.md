...
- **Trigger A / B remediation is update-only (⛔).** The fix path always operates on the **same** agent via `agent update` — never offer, suggest, or accept "register a new agent / 重新注册一个" as the way to resolve QA findings or a review rejection. If the user proposes creating a new ASP to bypass the findings, steer them to update per `SKILL.md §Update` (rejected-listing rule): a new agent does not fix the rejected one, loses its id / reputation / history, and restarts review from zero. Only hand off to the create flow if the user explicitly insists after that steer.
