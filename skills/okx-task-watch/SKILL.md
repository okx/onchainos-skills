...
- 🛑 Do NOT cancel, Ctrl-C, or kill an in-flight `okx-a2a user watch` call because it "looks hung". The call blocks until events arrive — silence = healthy long-poll, not a stuck process. Only stop on §Stop condition triggers.
...
4. On `alreadyHandled` → tell the user "this item was processed in another window"; **then re-enter `okx-a2a user watch --json`** (append the sticky `--job-id <X>` per §Session-scoped sticky if applicable) (the watch session continues — only the duplicate item is dropped). Do not execute the relay again.
5. Claim succeeded but relay failed → create a new `okx-a2a user notify` with the failure reason and a retry command; **do NOT** flip the original item back to pending. **Then re-enter `okx-a2a user watch --json`** (append the sticky `--job-id <X>` per §Session-scoped sticky if applicable).
