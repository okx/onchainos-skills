...
description: "Use this skill ONLY for EXTERNAL / third-party x402 payment-gated resources (arbitrary websites or APIs outside of onchainos). Trigger only when the user explicitly mentions 'x402' or 'sign x402 payment' for a non-onchainos URL / resource. If the user mentions 'onchainos', 'market', 'token', 'signal', 'trenches', 'memepump', or any /api/v6/dex/market path in their request — DO NOT use this skill; route to okx-dex-market / okx-dex-token / okx-dex-signal / okx-dex-trenches respectively (those skills own Market API payment handling end-to-end). Primary path signs via TEE with a wallet session (JWT) — recommended. Fallback path allows local EIP-3009 signing with the user's own private key only when the user explicitly opts in (key stays local but is not TEE-protected). Returns the payment proof (signature + authorization) that the caller can attach as a payment header to access the resource. Do NOT use for swap or token transfers — use okx-dex-swap. Do NOT use for wallet balance or portfolio queries — use okx-agentic-wallet or okx-wallet-portfolio. Do NOT use for security scanning — use okx-security. Do NOT use for transaction broadcasting — use okx-onchain-gateway. Do NOT use for general programming questions."
license: MIT
metadata:
  author: okx
  version: "1.0.4"
